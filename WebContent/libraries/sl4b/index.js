SL_BE = function(){this.REMOVAL_LIMIT=1000;this.m_bBrowserMapsLeakMemory=(navigator.userAgent.match(/MSIE/)!=undefined);};
SL_BE.prototype.createMap = function(){if(this.m_bBrowserMapsLeakMemory){var fClass=new Function();
fClass.deleteCount=0;return new fClass();
}else 
{return {};
}};
SL_BE.prototype.removeItem = function(B,A){delete B[A];if(this.m_bBrowserMapsLeakMemory){if(B.constructor.deleteCount++>=this.REMOVAL_LIMIT){var mNewMap=this.createMap();
for(sItem in B){mNewMap[sItem]=B[sItem];}B=mNewMap;}}return B;
};
SL_BE=new SL_BE();var SL4B_RttpProviderFactory=function(){};
if(false){function SL4B_RttpProviderFactory(){}
}SL4B_RttpProviderFactory = function(){this.m_oLastProvider=null;};
SL4B_RttpProviderFactory.prototype.createRttpProvider = SL_MF;function SL_MF(A){var l_oConfiguration=SL4B_Accessor.getConfiguration();
...
SL4B_StringEncoder=new SL4B_StringEncoder();function SL_BC(A){var i=function(){};
...
var SL4B_MD5=function(H){function SL_SM(A,B){return (A<<B)|(A>>>(32-B));
}
function SL_SQ(B,A){var lX4,lY4,lX8,lY8,lResult;
lX8=(B&0x80000000);lY8=(A&0x80000000);lX4=(B&0x40000000);lY4=(A&0x40000000);lResult=(B&0x3FFFFFFF)+(A&0x3FFFFFFF);if(lX4&lY4){return (lResult^0x80000000^lX8^lY8);
...
function SL_SP(C,B,A){return (C&B)|((~C)&A);
}
function SL_SK(C,B,A){return (C&A)|(B&(~A));
}
function SL_SI(C,B,A){return (C^B^A);
}
function SL_SL(C,B,A){return (B^(C|(~A)));
}
function SL_SO(B,E,C,A,G,F,D){B=SL_SQ(B,SL_SQ(SL_SQ(SL_SP(E,C,A),G),D));return SL_SQ(SL_SM(B,F),E);
}
function SL_SS(B,E,C,A,G,F,D){B=SL_SQ(B,SL_SQ(SL_SQ(SL_SK(E,C,A),G),D));return SL_SQ(SL_SM(B,F),E);
}
function SL_SN(B,E,C,A,G,F,D){B=SL_SQ(B,SL_SQ(SL_SQ(SL_SI(E,C,A),G),D));return SL_SQ(SL_SM(B,F),E);
}
function SL_ST(B,E,C,A,G,F,D){B=SL_SQ(B,SL_SQ(SL_SQ(SL_SL(E,C,A),G),D));return SL_SQ(SL_SM(B,F),E);
}
function SL_SH(A){var lWordCount;
var lMessageLength=A.length;
...
var lBytePosition=0;
var lByteCount=0;
while(lByteCount<lMessageLength){lWordCount=(lByteCount-(lByteCount%4))/4;lBytePosition=(lByteCount%4)*8;lWordArray[lWordCount]=(lWordArray[lWordCount]|(A.charCodeAt(lByteCount)<<lBytePosition));lByteCount++;}lWordCount=(lByteCount-(lByteCount%4))/4;lBytePosition=(lByteCount%4)*8;lWordArray[lWordCount]=lWordArray[lWordCount]|(0x80<<lBytePosition);lWordArray[lNumberOfWords-2]=lMessageLength<<3;lWordArray[lNumberOfWords-1]=lMessageLength>>>29;return lWordArray;
}
function SL_SJ(A){var WordToHexValue="",WordToHexValue_temp="",lByte,lCount;
for(lCount=0;lCount<=3;lCount++){lByte=(A>>>(lCount*8))&255;WordToHexValue_temp="0"+lByte.toString(16);WordToHexValue=WordToHexValue+WordToHexValue_temp.substr(WordToHexValue_temp.length-2,2);}return WordToHexValue;
}
function SL_SR(A){A=A.replace(/\r\n/g,"\n");var utftext="";
for(var n=0;n<A.length;n++){var c=A.charCodeAt(n);
if(c<128){utftext+=String.fromCharCode(c);}else 
if((c>127)&&(c<2048)){utftext+=String.fromCharCode((c>>6)|192);utftext+=String.fromCharCode((c&63)|128);}else 
...
H=SL_SR(H);x=SL_SH(H);a=0x67452301;b=0xEFCDAB89;c=0x98BADCFE;d=0x10325476;for(k=0;k<x.length;k+=16){AA=a;BB=b;CC=c;DD=d;a=SL_SO(a,b,c,d,x[k+0],S11,0xD76AA478);d=SL_SO(d,a,b,c,x[k+1],S12,0xE8C7B756);c=SL_SO(c,d,a,b,x[k+2],S13,0x242070DB);b=SL_SO(b,c,d,a,x[k+3],S14,0xC1BDCEEE);a=SL_SO(a,b,c,d,x[k+4],S11,0xF57C0FAF);d=SL_SO(d,a,b,c,x[k+5],S12,0x4787C62A);c=SL_SO(c,d,a,b,x[k+6],S13,0xA8304613);b=SL_SO(b,c,d,a,x[k+7],S14,0xFD469501);a=SL_SO(a,b,c,d,x[k+8],S11,0x698098D8);d=SL_SO(d,a,b,c,x[k+9],S12,0x8B44F7AF);c=SL_SO(c,d,a,b,x[k+10],S13,0xFFFF5BB1);b=SL_SO(b,c,d,a,x[k+11],S14,0x895CD7BE);a=SL_SO(a,b,c,d,x[k+12],S11,0x6B901122);d=SL_SO(d,a,b,c,x[k+13],S12,0xFD987193);c=SL_SO(c,d,a,b,x[k+14],S13,0xA679438E);b=SL_SO(b,c,d,a,x[k+15],S14,0x49B40821);a=SL_SS(a,b,c,d,x[k+1],S21,0xF61E2562);d=SL_SS(d,a,b,c,x[k+6],S22,0xC040B340);c=SL_SS(c,d,a,b,x[k+11],S23,0x265E5A51);b=SL_SS(b,c,d,a,x[k+0],S24,0xE9B6C7AA);a=SL_SS(a,b,c,d,x[k+5],S21,0xD62F105D);d=SL_SS(d,a,b,c,x[k+10],S22,0x2441453);c=SL_SS(c,d,a,b,x[k+15],S23,0xD8A1E681);b=SL_SS(b,c,d,a,x[k+4],S24,0xE7D3FBC8);a=SL_SS(a,b,c,d,x[k+9],S21,0x21E1CDE6);d=SL_SS(d,a,b,c,x[k+14],S22,0xC33707D6);c=SL_SS(c,d,a,b,x[k+3],S23,0xF4D50D87);b=SL_SS(b,c,d,a,x[k+8],S24,0x455A14ED);a=SL_SS(a,b,c,d,x[k+13],S21,0xA9E3E905);d=SL_SS(d,a,b,c,x[k+2],S22,0xFCEFA3F8);c=SL_SS(c,d,a,b,x[k+7],S23,0x676F02D9);b=SL_SS(b,c,d,a,x[k+12],S24,0x8D2A4C8A);a=SL_SN(a,b,c,d,x[k+5],S31,0xFFFA3942);d=SL_SN(d,a,b,c,x[k+8],S32,0x8771F681);c=SL_SN(c,d,a,b,x[k+11],S33,0x6D9D6122);b=SL_SN(b,c,d,a,x[k+14],S34,0xFDE5380C);a=SL_SN(a,b,c,d,x[k+1],S31,0xA4BEEA44);d=SL_SN(d,a,b,c,x[k+4],S32,0x4BDECFA9);c=SL_SN(c,d,a,b,x[k+7],S33,0xF6BB4B60);b=SL_SN(b,c,d,a,x[k+10],S34,0xBEBFBC70);a=SL_SN(a,b,c,d,x[k+13],S31,0x289B7EC6);d=SL_SN(d,a,b,c,x[k+0],S32,0xEAA127FA);c=SL_SN(c,d,a,b,x[k+3],S33,0xD4EF3085);b=SL_SN(b,c,d,a,x[k+6],S34,0x4881D05);a=SL_SN(a,b,c,d,x[k+9],S31,0xD9D4D039);d=SL_SN(d,a,b,c,x[k+12],S32,0xE6DB99E5);c=SL_SN(c,d,a,b,x[k+15],S33,0x1FA27CF8);b=SL_SN(b,c,d,a,x[k+2],S34,0xC4AC5665);a=SL_ST(a,b,c,d,x[k+0],S41,0xF4292244);d=SL_ST(d,a,b,c,x[k+7],S42,0x432AFF97);c=SL_ST(c,d,a,b,x[k+14],S43,0xAB9423A7);b=SL_ST(b,c,d,a,x[k+5],S44,0xFC93A039);a=SL_ST(a,b,c,d,x[k+12],S41,0x655B59C3);d=SL_ST(d,a,b,c,x[k+3],S42,0x8F0CCC92);c=SL_ST(c,d,a,b,x[k+10],S43,0xFFEFF47D);b=SL_ST(b,c,d,a,x[k+1],S44,0x85845DD1);a=SL_ST(a,b,c,d,x[k+8],S41,0x6FA87E4F);d=SL_ST(d,a,b,c,x[k+15],S42,0xFE2CE6E0);c=SL_ST(c,d,a,b,x[k+6],S43,0xA3014314);b=SL_ST(b,c,d,a,x[k+13],S44,0x4E0811A1);a=SL_ST(a,b,c,d,x[k+4],S41,0xF7537E82);d=SL_ST(d,a,b,c,x[k+11],S42,0xBD3AF235);c=SL_ST(c,d,a,b,x[k+2],S43,0x2AD7D2BB);b=SL_ST(b,c,d,a,x[k+9],S44,0xEB86D391);a=SL_SQ(a,AA);b=SL_SQ(b,BB);c=SL_SQ(c,CC);d=SL_SQ(d,DD);}var temp=SL_SJ(a)+SL_SJ(b)+SL_SJ(c)+SL_SJ(d);
...
}if(l_nListenerDebugLevel>=B){if(formattedMsg==null){if(date==null){date=new Date();}formattedMsg=SL_HU.formatDateStamp(date)+" - "+SL4B_DebugLevel.getDebugLevelName(B)+":   "+this.replaceMessageParameters(A,arguments,2);}l_oListener.logMessage(formattedMsg,B);}}};
...
}return SL_HU.formatDateStamp(C)+" - "+SL4B_DebugLevel.getDebugLevelName(A)+":   "+B;
};
...
SL_HU = new function(){this.m_pMonths=new Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec");this.m_pZeroPadding=new Array("","0","00","000");this.createDateStamp = function(){return this.formatDateStamp(new Date());
};
...
this.initialise = function(){if(this.m_bLoaded){setTimeout(SL_NS,0);}else 
{if(SL4B_Accessor.getConfiguration().isAutoLoadingEnabled()){SL4B_Accessor.getBrowserAdapter().addEventListener(window,"load",function(A){SL_NS(A);});}}SL4B_Accessor.getBrowserAdapter().addEventListener(window,"unload",SL_OR);SL4B_Accessor.getBrowserAdapter().addEventListener(window,"beforeunload",SL_QI);};
...
function SL_NS(A){SL4B_WindowEventHandler.onLoad(A);}
function SL_OR(A){SL4B_WindowEventHandler.onUnload(A);}
function SL_QI(A){SL4B_WindowEventHandler.onBeforeUnload(A);}
...
var l_oContainerFrame=eval(l_sContainerFrameLocation);
if(typeof l_oContainerFrame.SL4B_FrameRegistrar=="undefined"){l_oContainerFrame.SL4B_FrameRegistrar=new SL_LF();}this.m_oFrameRegistrar=l_oContainerFrame.SL4B_FrameRegistrar;}return this.m_oFrameRegistrar;
};
...
function SL_LF(){this.m_oMasterFrameRttpProvider=null;this.m_pRegisteredSlaveFrames=new Object();this.setMasterFrame = function(A){this.m_oMasterFrameRttpProvider=A;for(l_sFrameId in this.m_pRegisteredSlaveFrames){var l_oSlave=this.m_pRegisteredSlaveFrames[l_sFrameId];
...
SL4B_AbstractRttpProvider.prototype.loggedIn = SL_PZ;SL4B_AbstractRttpProvider.prototype.credentialsRetrieved = SL_ME;SL4B_AbstractRttpProvider.prototype.getObject = function(C,B,A){throw new SL4B_Error("getObject method not implemented");
};
...
SL4B_AbstractRttpProvider.prototype.addConnectionListener = SL_FO;SL4B_AbstractRttpProvider.prototype.removeConnectionListener = SL_FH;SL4B_AbstractRttpProvider.prototype.notifyConnectionListeners = SL_NQ;SL4B_AbstractRttpProvider.prototype.onUnload = SL_OT;function SL_AO(A){return SL4B_RttpUtils.getListener(A);
}
function SL_MS(){this.initialise();this.register();
try {SL4B_WindowEventHandler.addListener(this);}catch(e){SL4B_Accessor.getExceptionHandler().processException(e);}
if(!SL4B_Accessor.getConfiguration().isNoBrowserStatus()){this.addConnectionListener(new SL_EC());}}
function SL_QQ(){this.stop();if(this.m_fOnBeforeCloseEventHandler!=null){this.m_fOnBeforeCloseEventHandler();}}
...
function SL_PZ(){C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_LOGIN_OK_CONNECTION_EVENT));SL4B_SubscriptionManagerAccessor.ready();}
function SL_ME(){C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_CREDENTIALS_RETRIEVED_CONNECTION_EVENT,SL4B_Accessor.getCredentialsProvider().getUsername()));}
...
function SL_NQ(A){var l_pArguments=SL_NQ.arguments;
...
function SL_OT(A){SL4B_Accessor.getRttpProvider().internalStop();}
...
SL4B_ResilientRttpProvider.prototype._removePersistentActionKeyFromMap = function(A){if(this._isPersistentActionKeyInMap(A)){SL_BE.removeItem(this.m_mPersistentActionKeyMap,A.getId());return true;
}return false;
};
...
}SL4B_AbstractSubscriber.prototype.initialise = SL_AW;SL4B_AbstractSubscriber.prototype.getIdentifier = function(){return SL4B_SubscriptionManager.getListenerPrefix()+this.m_sListenerId;
};
...
SL4B_AbstractSubscriber.prototype.WTChat = SL_JP;SL4B_AbstractSubscriber.prototype.WTContribOk = SL_FX;SL4B_AbstractSubscriber.prototype.WTContribFailed = SL_KW;SL4B_AbstractSubscriber.prototype.WTCreateOk = SL_AK;SL4B_AbstractSubscriber.prototype.WTCreateFailed = SL_IP;SL4B_AbstractSubscriber.prototype.WTDeleteOk = SL_GV;SL4B_AbstractSubscriber.prototype.WTDeleteFailed = SL_LR;SL4B_AbstractSubscriber.prototype.WTDirUpdated = SL_HX;SL4B_AbstractSubscriber.prototype.WTFieldDeleted = SL_CW;SL4B_AbstractSubscriber.prototype.WTType2Clear = SL_SG;SL4B_AbstractSubscriber.prototype.WTType3Clear = SL_BU;SL4B_AbstractSubscriber.prototype.WTNewsUpdated = SL_NW;SL4B_AbstractSubscriber.prototype.WTObjectDeleted = SL_IV;SL4B_AbstractSubscriber.prototype.WTObjectInfo = SL_EP;SL4B_AbstractSubscriber.prototype.WTObjectNotFound = SL_HJ;SL4B_AbstractSubscriber.prototype.WTObjectNotStale = SL_AM;SL4B_AbstractSubscriber.prototype.WTObjectReadDenied = SL_KG;SL4B_AbstractSubscriber.prototype.WTObjectStale = SL_RE;SL4B_AbstractSubscriber.prototype.WTObjectStatus = SL_AS;SL4B_AbstractSubscriber.prototype.WTObjectType = SL_PM;SL4B_AbstractSubscriber.prototype.WTObjectUnavailable = SL_AG;SL4B_AbstractSubscriber.prototype.WTObjectUpdated = SL_LB;SL4B_AbstractSubscriber.prototype.WTObjectWriteDenied = SL_JG;SL4B_AbstractSubscriber.prototype.WTPageUpdated = SL_CS;SL4B_AbstractSubscriber.prototype.WTRecordMultiUpdated = SL_IG;SL4B_AbstractSubscriber.prototype.WTRecordUpdated = SL_DT;SL4B_AbstractSubscriber.prototype.WTStoryReset = SL_NO;SL4B_AbstractSubscriber.prototype.WTStoryUpdated = SL_NE;SL4B_AbstractSubscriber.prototype.WTStructureChange = SL_CQ;SL4B_AbstractSubscriber.prototype.WTStructureMultiChange = SL_BN;SL4B_AbstractSubscriber.prototype.WTPermissionUpdated = SL_DZ;SL4B_AbstractSubscriber.prototype.WTPermissionDeleted = SL_LJ;SL4B_AbstractSubscriber.prototype.dequeueNextMultiUpdate = SL_PV;SL4B_AbstractSubscriber.prototype.chat = function(C,B,D,A,E){this.methodNotImplemented("chat");};
...
function SL_AW(){this.m_sListenerId=SL4B_SubscriptionManager.addSubscriber(this);this.m_pMultiUpdateQueue=new Array();if(SL4B_SubscriptionManager.isReady()){C_CallbackQueue.addCallback(new Array(this,"ready"));}}
function SL_JP(C,B,D,A,E){C_CallbackQueue.addCallback(new Array(this,"chat",C,B,D,A,E));}
function SL_FX(B,A){C_CallbackQueue.addCallback(new Array(this,"contribOk",B,A));}
function SL_KW(B,D,C,A){C_CallbackQueue.addCallback(new Array(this,"contribFailed",B,D,C,A));}
function SL_AK(B,A){C_CallbackQueue.addCallback(new Array(this,"createOk",B,A));}
function SL_IP(B,D,C,A){C_CallbackQueue.addCallback(new Array(this,"createFailed",B,D,C,A));}
function SL_GV(B,A){C_CallbackQueue.addCallback(new Array(this,"deleteOk",B,A));}
function SL_LR(B,D,C,A){C_CallbackQueue.addCallback(new Array(this,"deleteFailed",B,D,C,A));}
function SL_HX(D,C,A,B){C_CallbackQueue.addCallback(new Array(this,"directoryUpdated",D,C,A,B));}
...
function SL_SG(A){C_CallbackQueue.addCallback(new Array(this,"type2Clear",A));}
function SL_BU(A){C_CallbackQueue.addCallback(new Array(this,"type3Clear",A));}
function SL_NW(A,D,C,B){C_CallbackQueue.addCallback(new Array(this,"newsUpdated",A,D,C,B));}
function SL_IV(A){C_CallbackQueue.addCallback(new Array(this,"objectDeleted",A));}
...
function SL_AM(A,B){C_CallbackQueue.addCallback(new Array(this,"objectNotStale",A,B));}
function SL_KG(A,B){C_CallbackQueue.addCallback(new Array(this,"objectReadDenied",A,B));}
function SL_RE(A,B){C_CallbackQueue.addCallback(new Array(this,"objectStale",A,B));}
function SL_AS(D,B,A,C){C_CallbackQueue.addCallback(new Array(this,"objectStatus",D,B,A,C));}
function SL_PM(C,B,A){C_CallbackQueue.addCallback(new Array(this,"objectType",C,B,A));}
function SL_AG(A){C_CallbackQueue.addCallback(new Array(this,"objectUnavailable",A));}
function SL_LB(A){C_CallbackQueue.addCallback(new Array(this,"objectUpdated",A));}
function SL_JG(A,B){C_CallbackQueue.addCallback(new Array(this,"objectWriteDenied",A,B));}
function SL_CS(E,D,B,A,C){C_CallbackQueue.addCallback(new Array(this,"pageUpdated",E,D,B,A,C));}
function SL_IG(A,B){this.m_pMultiUpdateQueue.push(new SL_HS(A,B,SL_HS.const_RECORD_UPDATE_TYPE,null));C_CallbackQueue.addCallback(new Array(this,"dequeueNextMultiUpdate"));}
function SL_DT(B,C,A){C_CallbackQueue.addCallback(new Array(this,"recordUpdated",B,C,A));}
function SL_NO(A){C_CallbackQueue.addCallback(new Array(this,"storyReset",A));}
function SL_NE(B,A){C_CallbackQueue.addCallback(new Array(this,"storyUpdated",B,A));}
function SL_CQ(A,C,D,B){C_CallbackQueue.addCallback(new Array(this,"structureChange",A,C,D,B));}
function SL_BN(B,A,D,C){this.m_pMultiUpdateQueue.push(new SL_HS(B,null,SL_HS.const_STRUCTURE_CHANGE_TYPE,new Array(A,D,C)));C_CallbackQueue.addCallback(new Array(this,"dequeueNextMultiUpdate"));}
function SL_DZ(A,B,C){this.m_pMultiUpdateQueue.push(new SL_HS(A,C,SL_HS.const_PERMISSION_UPDATE_TYPE,new Array(B)));C_CallbackQueue.addCallback(new Array(this,"dequeueNextMultiUpdate"));}
function SL_LJ(A,B){C_CallbackQueue.addCallback(new Array(this,"permissionDeleted",A,((!B) ? null : B)));}
function SL_PV(){var l_oMultiUpdate=this.m_pMultiUpdateQueue.shift();
if(l_oMultiUpdate!=null){if(l_oMultiUpdate.m_nType==SL_HS.const_RECORD_UPDATE_TYPE){this.recordMultiUpdated(l_oMultiUpdate.m_sObjectName,l_oMultiUpdate.m_oFieldData);}else 
if(l_oMultiUpdate.m_nType==SL_HS.const_PERMISSION_UPDATE_TYPE){this.permissionUpdated(l_oMultiUpdate.m_sObjectName,l_oMultiUpdate.m_pExtraData[0],l_oMultiUpdate.m_oFieldData);}else 
if(l_oMultiUpdate.m_nType==SL_HS.const_STRUCTURE_CHANGE_TYPE){this.structureMultiChange(l_oMultiUpdate.m_sObjectName,l_oMultiUpdate.m_pExtraData[0],l_oMultiUpdate.m_pExtraData[1],l_oMultiUpdate.m_pExtraData[2]);}else 
...
function SL_HS(C,D,B,A){this.m_sObjectName=C;this.m_oFieldData=D;this.m_nType=B;this.m_pExtraData=A;}
SL_HS.const_RECORD_UPDATE_TYPE=1;SL_HS.const_PERMISSION_UPDATE_TYPE=2;SL_HS.const_STRUCTURE_CHANGE_TYPE=3;var SL4B_RecordFieldData=function(){};
...
function SL_RJ(){}
SL_RJ.prototype = new SL4B_AbstractSubscriber;SL_RJ.prototype.getSubscriber = function(A){return this;
};
SL_RJ.prototype.ready = function(){};
SL_RJ.prototype.WTChat = function(C,B,D,A,E){};
SL_RJ.prototype.WTContribOk = function(A){};
SL_RJ.prototype.WTContribFailed = function(A,B){};
SL_RJ.prototype.WTdirUpdated = function(D,C,A,B){};
SL_RJ.prototype.WTDirMultiUpdated = function(A,B){};
SL_RJ.prototype.WTFieldDeleted = function(A,C,B){};
SL_RJ.prototype.WTType2Clear = function(A){};
SL_RJ.prototype.WTType3Clear = function(A){};
SL_RJ.prototype.WTNewsUpdated = function(A,D,C,B){};
SL_RJ.prototype.WTObjectDeleted = function(A){};
SL_RJ.prototype.WTObjectInfo = function(C,B,D,A){};
SL_RJ.prototype.WTObjectNotFound = function(A){};
SL_RJ.prototype.WTObjectNotStale = function(A,B){};
SL_RJ.prototype.WTObjectReadDenied = function(A,B){};
SL_RJ.prototype.WTObjectStale = function(A,B){};
SL_RJ.prototype.WTObjectStatus = function(D,B,A,C){};
SL_RJ.prototype.WTObjectType = function(C,B,A){};
SL_RJ.prototype.WTObjectUnavailable = function(A){};
SL_RJ.prototype.WTObjectUpdated = function(A){};
SL_RJ.prototype.WTObjectWriteDenied = function(A,B){};
SL_RJ.prototype.WTPageUpdated = function(E,D,B,A,C){};
SL_RJ.prototype.WTRecordMultiUpdated = function(A,B){};
SL_RJ.prototype.WTRecordUpdated = function(B,C,A){};
SL_RJ.prototype.WTStoryReset = function(A){};
SL_RJ.prototype.WTStoryUpdated = function(B,A){};
SL_RJ.prototype.WTStructureChange = function(A,C,D,B){};
SL_RJ.prototype.WTPermissionUpdated = function(B,A,C){};
SL_RJ.prototype.WTPermissionDeleted = function(B,A){};
var SL4B_SubscriptionManagerAccessor = new function(){this.m_bIsReady=false;this.m_pSubscriptionManagers=new Object();this.m_oNullSubscriptionManager=new SL_RJ();this.getSubscriptionManager = function(A){var l_oSubscriptionManager=this.m_pSubscriptionManagers[A];
...
SL4B_ExceptionHandler.prototype.processError = SL_JF;SL4B_ExceptionHandler.prototype.processException = SL_JE;function SL_JF(A){throw (A);
}
function SL_JE(A){throw (A);
}
...
function SL_LY(){SL4B_Accessor.setExceptionHandler(new SL4B_ExceptionHandler());}
...
var SL_MY = new function(){return true;
};
GF_CommonDomainExtractor = new function(){this.m_bDomainSet=false;this.m_sCommonDomain=null;this.getCommonDomain=SL_AQ;this.getLiberatorUrl=SL_OS;this.determineCommonDomain=SL_OB;this.getCommonDomainForDifferentPort=SL_RF;this.getDocumentDomain=SL_IR;};
function SL_AQ(){if(!this.m_bDomainSet){this.m_sCommonDomain=this.determineCommonDomain(this.getDocumentDomain(),window.location.href);this.m_bDomainSet=true;}return this.m_sCommonDomain;
}
function SL_OS(){var l_sUrl=SL4B_Accessor.getConfiguration().getJsContainerUrl();
if(l_sUrl==null){l_sUrl=SL4B_Accessor.getConfiguration().getServerUrl();}return l_sUrl;
}
function SL_OB(A,B){if(SL4B_Accessor.getConfiguration().allowCORS()){return null;
...
function SL_RF(A){SL4B_Logger.logConnectionMessage(true,"CommonDomainExtractor.getCommonDomainForDifferentPorts: web page and Liberator share the same domain ({0}), but their ports differ and domain needs to be changed.",A);var l_sCommonDomain=A.substring(A.indexOf(".")+1);
...
function SL_IR(){return document.domain;
}
...
SL4B_Configuration.prototype.getScriptTagAttribute = SL_AC;SL4B_Configuration.prototype.getIntegerScriptTagAttribute = SL_DQ;SL4B_Configuration.prototype.getPositiveIntegerScriptTagAttribute = SL_PK;SL4B_Configuration.prototype.getIntegerScriptTagAttributeInMilliseconds = SL_LK;SL4B_Configuration.prototype.unaryScriptTagAttributeExists = function(A){return (this.getScriptTagAttribute(A)!=null);
};
...
SL4B_Configuration.prototype.includeFlash = SL_KE;SL4B_Configuration.prototype.getRttpProvider = SL_NU;SL4B_Configuration.prototype.getDebugWindowType = SL_DD;SL4B_Configuration.prototype.isAutoLoadingEnabled = SL_NZ;SL4B_Configuration.prototype.yieldBeforeMessageProcessing = SL_LT;SL4B_Configuration.prototype.getCommonDomain = SL_QA;SL4B_Configuration.prototype.loadConfigurationFile = SL_AZ;SL4B_Configuration.prototype.loaded = SL_RI;SL4B_Configuration.prototype.getConnectionMethods = SL_PR;SL4B_Configuration.prototype.setPollingConnectionMethods = SL_PE;SL4B_Configuration.prototype.setStreamingConnectionMethods = SL_GO;SL4B_Configuration.prototype.getPollingConnectionMethods = SL_CP;SL4B_Configuration.prototype.getStreamingConnectionMethods = SL_RN;SL4B_Configuration.prototype.getType3PollPeriod = SL_QE;SL4B_Configuration.prototype.getType5ReconnectCount = SL_HA;SL4B_Configuration.prototype.getType5PadLength = SL_FI;SL4B_Configuration.prototype.getConnectionTimeout = SL_IH;SL4B_Configuration.prototype.getNOOPInterval = SL_IA;SL4B_Configuration.prototype.getNOOPTimeout = SL_FC;SL4B_Configuration.prototype.getRequestChannelResponseTimeout = SL_GH;SL4B_Configuration.prototype.getLoginTimeout = SL_MN;SL4B_Configuration.prototype.getMaxGetLength = SL_QJ;SL4B_Configuration.prototype.getApplicationId = function(){var l_sClientApplicationId=this.getScriptTagAttribute("applicationid");
...
function SL_PK(A){A=A.toLowerCase();var l_nPositiveIntegerValue=this.getAttribute(A);
var l_sValue=this.m_oIndexScript.getAttribute(A);
...
function SL_LK(A){A=A.toLowerCase();var l_nAttributeValue=this.getIntegerScriptTagAttribute(A);
...
function SL_KE(){return (this.includeRtml()||this.unaryScriptTagAttributeExists("enableflash"));
}
function SL_PE(A,B){if(!SL4B_Browser.isKnownBrowser(A)){throw new SL4B_Error("Configuration method setPollingConnectionMethods called with invalid SL4B_Browser parameter: "+A);
...
function SL_RN(){if(this.isStreamingDisabled()){return [];
}else 
{var l_oBrowser=SL4B_Accessor.getBrowserAdapter().getBrowser();
...
function SL_PR(){var l_sConnectionTypes=this.getScriptTagAttribute("connectiontypes");
...
function SL_QE(){return this.getIntegerScriptTagAttribute("type3pollperiod");
}
...
function SL_IH(){return this.getPositiveIntegerScriptTagAttribute("connectiontimeout");
}
function SL_IA(){return this.getPositiveIntegerScriptTagAttribute("noopinterval");
}
...
function SL_MN(){return this.getPositiveIntegerScriptTagAttribute("logintimeout");
}
function SL_QJ(){return this.getIntegerScriptTagAttribute("maxgetlength");
}
function SL_NU(){return this.getScriptTagAttribute("rttpprovider").toLowerCase();
}
function SL_DD(){return this.getScriptTagAttribute("debugwindowtype");
}
function SL_NZ(){var l_vEnableAutoLoading=this.getScriptTagAttribute("enableautoloading");
...
function SL_LT(){var l_vYield=this.getScriptTagAttribute("yieldbeforemessageprocessing");
if(typeof l_vYield=="string"){l_vYield=(l_vYield.toLowerCase()=="true");}return l_vYield;
}
function SL_QA(){var l_sCommonDomain=this.getScriptTagAttribute("commondomain");
if(l_sCommonDomain==null){l_sCommonDomain=GF_CommonDomainExtractor.getCommonDomain();}return l_sCommonDomain;
}
function SL_AZ(){var l_sConfigurationFileUrl=this.getScriptTagAttribute("configurationfile");
...
function SL_HN(){SL4B_MethodInvocationProxy=new SL4B_MethodInvocationProxy();}
...
SL4B_StandardCredentialsProvider.prototype = new SL4B_AbstractCredentialsProvider;SL4B_StandardCredentialsProvider.prototype.getCredentials = SL_BK;SL4B_StandardCredentialsProvider.prototype.getUsername = SL_MU;function SL_BK(A){var l_oConfiguration=SL4B_Accessor.getConfiguration();
A.login(l_oConfiguration.getUsername(),l_oConfiguration.getPassword());}
function SL_MU(){return SL4B_Accessor.getConfiguration().getUsername();
}
function SL_RV(){SL4B_Accessor.setCredentialsProvider(new SL4B_StandardCredentialsProvider());}
...
function SL_SU(){if(iframeLoader==null){iframeLoader=document.createElement('div');iframeLoader.setAttribute("id","iframeloader");document.body.appendChild(iframeLoader);}return iframeLoader;
}
...
if(typeof l_oIFrameElement!="undefined"){SL_SU().removeChild(l_oIFrameElement);}SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"Creating IFRAME for {0} at URL {1} with a timeout of {2}ms",C,D,A);l_oIFrameElement=document.createElement('iframe');l_oIFrameElement.src=D;l_oIFrameElement.style.display="none";l_oIFrameIdentifierToElementMap[C]=l_oIFrameElement;SL_SU().appendChild(l_oIFrameElement);var self=this;
...
if(typeof l_oIFrameElement!="undefined"){SL_SU().removeChild(l_oIFrameElement);}delete (l_oIFrameIdentifierToElementMap[A]);delete (l_oIFrameIdentifierToTimeoutMap[A]);};
};
...
}SL4B_KeyMasterCredentialsProvider = function(){this.m_sSourceId="default";this.m_oRequestManager=null;this.m_sUserName="";this.m_oRttpProvider=null;this.reconnectAttempt=0;this.doneBackwardsCompatibleCheck=false;this.storedRequests=[];this.m_sKeyMasterToken=null;this.iFrameLoaded=false;this.m_bPreFetching=false;this.m_bPreFetchAvailable=false;this.m_sPreFetchUsername=null;this.m_sPreFetchPassword=null;this.m_oPreFetchTimestamp=null;this.m_oPreFetchCallback=null;if(SL4B_Accessor.getConfiguration().getKeyMasterUrl()==null){SL4B_Logger.log(SL4B_DebugLevel.const_ERROR_INT,"SL4B_KeyMasterCredentialsProvider: KeyMaster URL has not been defined");}};
SL4B_KeyMasterCredentialsProvider.prototype = new SL4B_AbstractCredentialsProvider;SL4B_KeyMasterCredentialsProvider.prototype.getCredentials = SL_SB;SL4B_KeyMasterCredentialsProvider.prototype.getKeyMasterToken = SL_IM;SL4B_KeyMasterCredentialsProvider.prototype.getUsername = SL_GW;SL4B_KeyMasterCredentialsProvider.prototype.setPreFetching = function(){this.m_bPreFetching=true;};
...
}SL4B_SlaveFrameRttpProvider = function(){SL4B_AbstractRttpProvider.apply(this);this.m_oMasterFrameRttpProvider=null;this.m_oRttpCommandQueue=new GF_RttpCommandQueue();this.m_bMasterRefresh=false;this.m_pConnectionListeners=[];this.m_sSlaveFrameId=SL4B_Accessor.getConfiguration().getFrameId();};
...
SL4B_SlaveFrameRttpProvider.prototype.register = SL_PN;SL4B_SlaveFrameRttpProvider.prototype.masterRegistered = SL_KR;SL4B_SlaveFrameRttpProvider.prototype.masterClosing = SL_HL;SL4B_SlaveFrameRttpProvider.prototype.connect = function(){};
SL4B_SlaveFrameRttpProvider.prototype.login = function(A,B){};
SL4B_SlaveFrameRttpProvider.prototype.reconnect = function(){};
SL4B_SlaveFrameRttpProvider.prototype.getObject = SL_EG;SL4B_SlaveFrameRttpProvider.prototype.getObjects = SL_OV;SL4B_SlaveFrameRttpProvider.prototype.removeObject = SL_OD;SL4B_SlaveFrameRttpProvider.prototype.removeObjects = SL_EX;SL4B_SlaveFrameRttpProvider.prototype.removeSubscriber = SL_GK;SL4B_SlaveFrameRttpProvider.prototype.getObjectType = SL_QN;SL4B_SlaveFrameRttpProvider.prototype.setThrottleObject = SL_JH;SL4B_SlaveFrameRttpProvider.prototype.setThrottleObjects = SL_QL;SL4B_SlaveFrameRttpProvider.prototype.setGlobalThrottle = SL_RQ;SL4B_SlaveFrameRttpProvider.prototype.createObject = SL_GS;SL4B_SlaveFrameRttpProvider.prototype.contribObject = SL_CM;SL4B_SlaveFrameRttpProvider.prototype.deleteObject = SL_DV;SL4B_SlaveFrameRttpProvider.prototype.cancelPersistedAction = SL_DO;SL4B_SlaveFrameRttpProvider.prototype.resendPersistedAction = SL_RW;SL4B_SlaveFrameRttpProvider.prototype.getFieldNames = SL_HQ;SL4B_SlaveFrameRttpProvider.prototype.logout = function(){};
...
function SL_PN(){if(!SL4B_Accessor.getConfiguration()._$isRegistrationWindowEnabled()){setTimeout(this._getRegisterSlaveFrameFunction(),0);}}
...
function SL_KR(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"SlaveFrameRttpProvider.masterRegistered: {0}",A);this.m_oMasterFrameRttpProvider=A;var l_nConnectionListeners=this.m_pConnectionListeners.length;
...
function SL_HL(){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"SlaveFrameRttpProvider.masterClosing");this.m_oMasterFrameRttpProvider=null;this.m_bMasterRefresh=true;}
var SL4B_SlaveFrameWrapperRttpProvider=function(){};
if(false){function SL4B_SlaveFrameWrapperRttpProvider(){}
}SL4B_SlaveFrameWrapperRttpProvider = function(){SL4B_ResilientRttpProvider.apply(this);this.m_oBaseRttpProvider=null;this.m_bProviderReady=false;this.m_oConnectionListener=new SL_MT(this);this.setUnderlyingRttpProvider = function(A){if(this.m_oBaseRttpProvider!=null){
...
this.initialise();};
function SL_MT(B){this.m_pConnectionListeners=new Array();this.m_oSlaveProviderWrapper=B;this.m_oRttpProvider=null;this.m_oSubscriptionManager=null;this.setRttpProvider = function(A){this.m_oRttpProvider=A;};
}
SL_MT.prototype = new SL4B_ConnectionListener;SL_MT.prototype.registerSubscriptionManager = function(A){this.m_oSubscriptionManager=A;};
SL_MT.prototype.addConnectionListener = function(A){this.m_pConnectionListeners.push(A);};
SL_MT.prototype.removeConnectionListener = function(A){var l_nMatchIndex=-1;
for(var l_nListener=0,l_nLength=this.m_pConnectionListeners.length;l_nListener<l_nLength;++l_nListener){if(this.m_pConnectionListeners[l_nListener]==A){l_nMatchIndex=l_nListener;break;
}}if(l_nMatchIndex!=-1){this.m_pConnectionListeners.splice(l_nMatchIndex,1);}return (l_nMatchIndex!=-1);
};
SL_MT.prototype.notifyConnectionListeners = function(A,B){for(var l_nListener=0,l_nLength=this.m_pConnectionListeners.length;l_nListener<l_nLength;++l_nListener){if(B.length>0&&B[0]===undefined){B=[];}SL4B_MethodInvocationProxy._invokeWithTryCatch(this.m_pConnectionListeners[l_nListener],A,B);}};
SL_MT.prototype.connectionAttempt = function(B,A){this.notifyConnectionListeners("connectionAttempt",[B,A]);};
SL_MT.prototype.connectionWarning = function(C,D,B,A){this.notifyConnectionListeners("connectionWarning",[C,D,B,A]);};
SL_MT.prototype.connectionError = function(A){this.notifyConnectionListeners("connectionError",[A]);};
SL_MT.prototype.connectionInfo = function(A){this.notifyConnectionListeners("connectionInfo",[A]);};
SL_MT.prototype.connectionOk = function(C,A,B){this.notifyConnectionListeners("connectionOk",[C,A,B]);};
SL_MT.prototype.reconnectionOk = function(C,A,B){this.notifyConnectionListeners("reconnectionOk",[C,A,B]);};
SL_MT.prototype.fileDownloadError = function(B,A){this.notifyConnectionListeners("fileDownloadError",[B,A]);};
SL_MT.prototype.credentialsRetrieved = function(A){this.notifyConnectionListeners("credentialsRetrieved",[A]);};
SL_MT.prototype.loginError = function(A){this.notifyConnectionListeners("loginError",[A]);};
SL_MT.prototype.credentialsProviderSessionError = function(A,D,C,B){this.notifyConnectionListeners("credentialsProviderSessionError",[A,D,C,B]);};
SL_MT.prototype.loginOk = function(){this.notifyConnectionListeners("loginOk",[]);SL4B_WindowRegistrar.loginOK();setTimeout(this._invokeSendQueuedCommandsOnTimeout(),0);if(this.m_oSubscriptionManager!=null){this.m_oSubscriptionManager.ready();this.m_oSubscriptionManager=null;}};
SL_MT.prototype._invokeSendQueuedCommandsOnTimeout = function(){return function(){SL4B_Accessor.getRttpProvider()._$sendQueuedCommands();};
};
SL_MT.prototype.message = function(A,B){this.notifyConnectionListeners("message",[A,B]);};
SL_MT.prototype.serviceMessage = function(A,D,B,C){this.notifyConnectionListeners("serviceMessage",[A,D,B,C]);};
SL_MT.prototype.sessionEjected = function(A,B){this.notifyConnectionListeners("sessionEjected",[A,B]);};
SL_MT.prototype.sourceMessage = function(A,C,D,B){this.notifyConnectionListeners("sourceMessage",[A,C,D,B]);};
SL_MT.prototype.statistics = function(A,D,E,B,C){this.notifyConnectionListeners("statistics",[A,D,E,B,C]);};
...
function SL_JV(){SL4B_ZunAlgorithm.prototype = new SL4B_FailoverAlgorithm;}
...
}SL4B_FailoverRttpProvider = function(){SL4B_ResilientRttpProvider.apply(this);this.m_bConnectionListenerAdded=false;this.m_oConnectionListener=new SL_IJ(this);this.m_bAttemptReconnections=true;this.m_bConnectionLost=false;var l_sUrl=SL4B_Accessor.getConfiguration().getService();
...
function SL_IJ(B){this.m_pConnectionListeners=new Array();this.m_oFailoverProvider=B;this.m_oRttpProvider=null;this.setRttpProvider = function(A){this.m_oRttpProvider=A;};
}
SL_IJ.prototype = new SL4B_ConnectionListener;SL_IJ.prototype.addConnectionListener = function(A){this.m_pConnectionListeners.push(A);};
SL_IJ.prototype.removeConnectionListener = function(A){var l_nMatchIndex=-1;
for(var l_nListener=0,l_nLength=this.m_pConnectionListeners.length;l_nListener<l_nLength;++l_nListener){if(this.m_pConnectionListeners[l_nListener]==A){l_nMatchIndex=l_nListener;break;
}}if(l_nMatchIndex!=-1){this.m_pConnectionListeners.splice(l_nMatchIndex,1);}return (l_nMatchIndex!=-1);
};
SL_IJ.prototype.notifyConnectionListeners = function(A,B){for(var l_nListener=0,l_nLength=this.m_pConnectionListeners.length;l_nListener<l_nLength;++l_nListener){if(B.length>0&&B[0]===undefined){B=[];}SL4B_MethodInvocationProxy._invokeWithTryCatch(this.m_pConnectionListeners[l_nListener],A,B);}};
SL_IJ.prototype.connectionAttempt = function(B,A){this.notifyConnectionListeners("connectionAttempt",[B,A]);};
SL_IJ.prototype.connectionWarning = function(C,D,B,A){this.notifyConnectionListeners("connectionWarning",[C,D,B,A]);};
SL_IJ.prototype.connectionError = function(A){if(this.m_oFailoverProvider.m_bAttemptReconnections){var l_bIsAnotherServerAvailable=this.m_oFailoverProvider.nextRttpProvider();
...
SL_IJ.prototype.connectionInfo = function(A){this.notifyConnectionListeners("connectionInfo",[A]);};
SL_IJ.prototype.connectionOk = function(C,A,B){this.notifyConnectionListeners("connectionOk",[C,A,B]);};
SL_IJ.prototype.reconnectionOk = function(C,A,B){this.notifyConnectionListeners("reconnectionOk",[C,A,B]);};
SL_IJ.prototype.fileDownloadError = function(B,A){this.notifyConnectionListeners("fileDownloadError",[B,A]);};
SL_IJ.prototype.credentialsRetrieved = function(A){this.notifyConnectionListeners("credentialsRetrieved",[A]);};
SL_IJ.prototype.loginError = function(A){this.notifyConnectionListeners("loginError",[A]);};
SL_IJ.prototype.credentialsProviderSessionError = function(A,D,C,B){this.notifyConnectionListeners("credentialsProviderSessionError",[A,D,C,B]);};
SL_IJ.prototype.loginOk = function(){this.notifyConnectionListeners("loginOk",[]);var self=this;
setTimeout(function(){self.m_oFailoverProvider._$sendQueuedCommands();},0);};
SL_IJ.prototype.message = function(A,B){this.notifyConnectionListeners("message",[A,B]);};
SL_IJ.prototype.serviceMessage = function(A,D,B,C){this.notifyConnectionListeners("serviceMessage",[A,D,B,C]);};
SL_IJ.prototype.sessionEjected = function(A,B){this.notifyConnectionListeners("sessionEjected",[A,B]);};
SL_IJ.prototype.sourceMessage = function(A,C,D,B){this.notifyConnectionListeners("sourceMessage",[A,C,D,B]);};
SL_IJ.prototype.statistics = function(A,D,E,B,C){this.notifyConnectionListeners("statistics",[A,D,E,B,C]);};
...
SL4B_AbstractJavaRttpProvider.prototype.clearObjectListeners = SL_NB;SL4B_AbstractJavaRttpProvider.prototype.blockObjectListeners = SL_DH;SL4B_AbstractJavaRttpProvider.prototype.unblockObjectListeners = SL_DE;SL4B_AbstractJavaRttpProvider.prototype.createObject = function(A,B){return this.m_oRttpApplet.createObject(A,B);
};
...
SL4B_AbstractJavaRttpProvider.prototype.setRttpApplet = SL_GC;SL4B_AbstractJavaRttpProvider.prototype.getAppletParameterHtml = SL_GR;SL4B_AbstractJavaRttpProvider.prototype.writeAppletToDocument = SL_NH;SL4B_AbstractJavaRttpProvider.prototype.startAppletCheckTimeout = SL_KK;SL4B_AbstractJavaRttpProvider.prototype.startRttpAppletTimeout = SL_IL;SL4B_AbstractJavaRttpProvider.prototype.appletCheckDownloadComplete = SL_NY;SL4B_AbstractJavaRttpProvider.prototype.rttpAppletDownloadComplete = SL_JU;SL4B_AbstractJavaRttpProvider.prototype.urlCheckDownloadFailed = SL_DK;SL4B_AbstractJavaRttpProvider.prototype.appletCheckDownloadFailed = SL_RU;SL4B_AbstractJavaRttpProvider.prototype.rttpAppletDownloadFailed = SL_LC;SL4B_AbstractJavaRttpProvider.prototype.queueUpdates = SL_GL;SL4B_AbstractJavaRttpProvider.prototype.dequeueUpdates = SL_PL;SL4B_AbstractJavaRttpProvider.prototype.WTConnectionError = SL_FV;SL4B_AbstractJavaRttpProvider.prototype.WTConnectionInfo = SL_AA;SL4B_AbstractJavaRttpProvider.prototype.WTConnectionOk = SL_BY;SL4B_AbstractJavaRttpProvider.prototype.WTMessage = SL_GF;SL4B_AbstractJavaRttpProvider.prototype.WTReconnect = SL_JO;SL4B_AbstractJavaRttpProvider.prototype.WTServiceMessage = SL_HI;SL4B_AbstractJavaRttpProvider.prototype.WTSessionEjected = SL_GN;SL4B_AbstractJavaRttpProvider.prototype.WTSourceMessage = SL_KT;SL4B_AbstractJavaRttpProvider.prototype.WTStats = SL_AV;function SL_FZ(){this.m_oRttpApplet.addStatsListener('SL4B_Accessor.getRttpProvider()');this.m_oRttpApplet.addConnectionListener("SL4B_Accessor.getRttpProvider()");this.m_oRttpApplet.addReconnectListener('SL4B_Accessor.getRttpProvider()');this.m_oRttpApplet.addMessagesListener('SL4B_Accessor.getRttpProvider()');this.m_oRttpApplet.connect(false);}
...
{this.loggedIn();}}
function SL_NB(B,A){if(typeof A=="undefined"){this.m_oRttpApplet.clearObjectListeners(this.getListener(B));}else 
...
function SL_NH(A){var l_sHtmlToWrite="<div style=\"height:0px;width:0px;overflow:hidden;\">";
l_sHtmlToWrite+=A;l_sHtmlToWrite+="</div>";document.write(l_sHtmlToWrite);}
function SL_KK(){this.m_nAppletCheckTimeoutHandle=setTimeout("SL4B_Accessor.getRttpProvider().appletCheckDownloadFailed()",SL4B_Accessor.getConfiguration().getAppletCheckTimeout());}
function SL_IL(){this.m_nRttpAppletTimeoutHandle=setTimeout("SL4B_Accessor.getRttpProvider().rttpAppletDownloadFailed()",SL4B_Accessor.getConfiguration().getRttpAppletTimeout());}
function SL_NY(){if(this.m_nAppletCheckTimeoutHandle!==null){clearTimeout(this.m_nAppletCheckTimeoutHandle);this.m_nAppletCheckTimeoutHandle=null;}}
function SL_JU(){if(this.m_nRttpAppletTimeoutHandle!==null){clearTimeout(this.m_nRttpAppletTimeoutHandle);this.m_nRttpAppletTimeoutHandle=null;}var l_oApplet=SL4B_Accessor.getBrowserAdapter().getElementById(this.getRttpAppletId());
...
function SL_RU(){if(this.m_nAppletCheckTimeoutHandle!==null){this.m_nAppletCheckTimeoutHandle=null;this.notifyConnectionListeners(this.const_FILE_DOWNLOAD_ERROR_CONNECTION_EVENT,SL4B_FileType.APPLET_CHECK,this.m_sAppletDirectoryUrl+"/AppletCheck.class");}}
function SL_LC(){if(this.m_nRttpAppletTimeoutHandle!==null){this.m_nRttpAppletTimeoutHandle=null;var l_oConfiguration=SL4B_Accessor.getConfiguration();
...
function SL_PL(){var l_oQueue=this.m_pUpdateQueue.shift();
...
function SL_BY(C,A,B){if(!this.m_bConnectionEstablished){this.m_bConnectionEstablished=true;setTimeout("SL4B_Accessor.getRttpProvider().connected();",0);}C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_OK_CONNECTION_EVENT,C,A,B));}
...
function SL_JO(){C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_ERROR_CONNECTION_EVENT));}
...
function SL_KT(A,C,D,B){C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_SOURCE_MESSAGE_CONNECTION_EVENT,A,C,D,B));}
function SL_AV(A,D,E,B,C){C_CallbackQueue.addCallback(new Array(this,"notifyConnectionListeners",this.const_STATISTICS_CONNECTION_EVENT,A,D,E,B,C));}
...
SL4B_AppletRttpProvider.prototype = new SL4B_AbstractJavaRttpProvider;SL4B_AppletRttpProvider.prototype.initialise = SL_KO;SL4B_AppletRttpProvider.prototype.loadUrlCheck = SL_HD;SL4B_AppletRttpProvider.prototype.loadAppletCheck = SL_OQ;SL4B_AppletRttpProvider.prototype.loadRttpApplet = SL_LA;function SL_KO(){this.loadUrlCheck();this.loadAppletCheck();this.loadRttpApplet();}
...
function SL_OQ(){var l_sHtml="<applet width=\"0\" height=\"0\"";
...
function SL_LA(){var l_oConfiguration=SL4B_Accessor.getConfiguration();
...
SL4B_AbstractConnection.prototype.send = SL_JD;SL4B_AbstractConnection.prototype.setRequestHttpRequest = SL_JR;SL4B_AbstractConnection.prototype.setResponseHttpRequest = SL_MP;SL4B_AbstractConnection.prototype.getLastSequenceNumber = function(){return this.m_sLastSequenceNumber;
};
...
SL4B_AbstractConnection.prototype.setMessageReceiver = SL_QV;SL4B_AbstractConnection.prototype.parseRttpMessage = SL_OZ;SL4B_AbstractConnection.prototype.createChannel = SL_QF;SL4B_AbstractConnection.prototype.createStandardRequestChannel = SL_LN;SL4B_AbstractConnection.prototype.createStandardResponseChannel = SL_II;SL4B_AbstractConnection.prototype.getResponseUniqueId = SL_QO;SL4B_AbstractConnection.prototype.getRequestUniqueId = SL_PB;function SL_QO(){return this.m_sResponseUniqueId;
}
function SL_PB(){return this.m_sRequestUniqueId;
}
function SL_JD(A){if(!this.m_bConnectionStopped){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"AbstractConnection.send({0}?{1})",this.m_sUrlPrefix,A);
...
function SL_MP(A){this.m_oResponseHttpRequest=A;}
function SL_JR(A){this.m_oRequestHttpRequest=A;}
function SL_QV(A){if(A!==null&&typeof A.receiveMessage!="function"){throw new SL4B_Exception("Specified listener does not define the receiveMessage method");
...
function SL_SV(A){var sCode=A.slice(0,A.length-1);
var sField=oRttpProvider.getFieldName(sCode);
return (sField) ? sField+"=" : A;
}
B=B.replace(/\w+=/g,SL_SV);return B;
};
function SL_OZ(A){if(!this.m_bConnectionStopped){if(this.m_bDecodeFields===null){var bDecodeFields=SL4B_Accessor.getConfiguration().getDecodeFields();
...
function SL_QF(B,A){C_CallbackQueue.addCallback(new Array(SL4B_Accessor.getUnderlyingRttpProvider(),"notifyConnectionListeners",this.m_oRttpProvider.const_INFO_CONNECTION_EVENT,"Establishing "+B.replace(/^frm/,"").toLowerCase()+" channel (URL: "+A+")"));var l_sUniqueId=(new Date()).valueOf();
...
function SL_LN(){var l_sUrlPrefix=this.m_oRttpProvider.getJsContainerUrl()+"/";
...
function SL_II(){var l_sUrlPrefix=this.m_oRttpProvider.getJsContainerUrl()+"/";
...
}GF_SlidingStatisticsWindow.prototype = SL_BC(GF_SlidingWindow);GF_SlidingStatisticsWindow.prototype.clear = function(){GF_SlidingWindow.prototype.clear.call(this);this.m_nMin=NaN;this.m_nMax=NaN;this.m_nTotal=0;this.m_nSquaredTotal=0;};
...
SL4B_ConnectionProxy.prototype.connect = SL_BI;SL4B_ConnectionProxy.prototype.send = SL_EU;SL4B_ConnectionProxy.prototype.setRequestHttpRequest = SL_PD;SL4B_ConnectionProxy.prototype.setResponseHttpRequest = SL_LW;SL4B_ConnectionProxy.prototype.start = SL_PG;SL4B_ConnectionProxy.prototype.stop = SL_CN;SL4B_ConnectionProxy.prototype.setMessageReceiver = SL_DN;SL4B_ConnectionProxy.prototype.setConnection = SL_MW;SL4B_ConnectionProxy.prototype.parseRttpMessage = SL_DG;SL4B_ConnectionProxy.prototype.getResponseUniqueId = SL_HZ;SL4B_ConnectionProxy.prototype.getRequestUniqueId = SL_KP;function SL_HZ(){this.verifyConnection();return this.m_oConnection.getResponseUniqueId();
}
function SL_KP(){this.verifyConnection();return this.m_oConnection.getRequestUniqueId();
}
...
function SL_BI(){SL4B_Logger.logConnectionMessage(false,"ConnectionProxy.connect()");this.verifyConnection();this.m_oConnection.connect();}
...
function SL_PG(){SL4B_Logger.logConnectionMessage(false,"ConnectionProxy.start()");this.verifyConnection();this.m_oConnection.start();}
...
function SL_MW(A){if(this.m_oConnection!=null){this.m_oConnection.stop();this.stopClockSync();}SL4B_Logger.logConnectionMessage(false,"ConnectionProxy.setConnection: {0}",A);this.m_oConnection=A;this.m_oConnection.setMessageReceiver(this.m_oMessageReceiver);this._addExistingResponseQueueStatisticsListenerToNewConnection(A);var sClockSyncStrategyClass=SL4B_Accessor.getConfiguration().getClockSyncStrategy();
...
SL4B_ObjectCache.const_FIELD_NAME_DELIMITER=",";SL4B_ObjectCache.const_OBJECT_NAME_AND_FIELD_LIST_DELIMITER=";";SL4B_ObjectCache.const_BLANK_STRING="";SL4B_ObjectCache.SUBJECT_DELETED="deleted";SL4B_ObjectCache.SUBJECT_ADDED="added";SL4B_ObjectCache.prototype.receiveMessage = SL_QR;SL4B_ObjectCache.prototype.processRttpMessage = SL4B_ObjectCache.prototype.receiveMessage;SL4B_ObjectCache.prototype.requestObject = SL_EH;SL4B_ObjectCache.prototype.sendCachedFields = SL_DX;SL4B_ObjectCache.prototype.requestObjects = SL_EN;SL4B_ObjectCache.prototype.discardObject = SL_HT;SL4B_ObjectCache.prototype.discardObjects = SL_OO;SL4B_ObjectCache.prototype.cacheObject = SL_JL;SL4B_ObjectCache.prototype.cacheType1Record = SL_DR;SL4B_ObjectCache.prototype.cacheType2Record = SL_OY;SL4B_ObjectCache.prototype.cacheType3Record = SL_PS;SL4B_ObjectCache.prototype.deleteType2RecordLevel = SL_SD;SL4B_ObjectCache.prototype.type2Clear = SL_FB;SL4B_ObjectCache.prototype.type3Clear = SL_PY;SL4B_ObjectCache.prototype.notFound = SL_EE;SL4B_ObjectCache.prototype.cacheDirectory = SL_AR;SL4B_ObjectCache.prototype.statusUpdated = SL_QP;SL4B_ObjectCache.prototype.sendObjectAction = SL_QB;SL4B_ObjectCache.prototype.createObjectArray = SL_EI;SL4B_ObjectCache.prototype.cacheContainer = SL_SC;SL4B_ObjectCache.prototype.cacheNewsHeadline = SL_DS;SL4B_ObjectCache.prototype.cacheNewsStory = SL_RA;SL4B_ObjectCache.prototype.cachePermission = SL_OU;SL4B_ObjectCache.prototype.clearPermission = SL_MC;SL4B_ObjectCache.prototype.deletePermissionEntry = SL_CF;SL4B_ObjectCache.prototype.parseOrderChanges = SL_JK;SL4B_ObjectCache.prototype.isActive = SL_DW;SL4B_ObjectCache.prototype.clearContainer = SL_QZ;SL4B_ObjectCache.prototype.clear = function(){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.clear()");this.m_pObjects=new Object();this.m_pObjectDataCache=new Object();this.m_pObjectDataCacheType2=new Object();this.m_pObjectDataCacheType3=new Object();this.m_pObjectNumberToObjectKeyMap=new Object();this.m_pDirectoryCache=new Object();this.m_pStoryCache=new Object();this.m_pNewsHeadlineCache=new Object();this.m_pContainerCache=new Object();this.m_pPermissionCache=new Object();this.m_mChatCache=new Object();};
function SL_QR(C,A,B){var l_nRttpCode=C.getRttpCode();
switch(l_nRttpCode){
...
function SL_QB(C,B,A,D){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.sendObjectAction({0}, {1})",C,B);if(B.length==0){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINE_INT,"ObjectCache.sendObjectAction. Not performing {0} action as there are no subjects to send to Liberator, not sending any messages.",C);return;
...
function SL_HT(B,A){var l_sSendFilter=A;
...
function SL_OO(A,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.discardObjects({0}, {1})",A,B);var l_sSendFilter=B;
...
function SL_JL(C,A,B){var oNormalisedRttpMessage=C;
...
function SL_OY(A,C,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.cacheType2Record({0}, {1}, {2})",A,C,B);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_PS(A,C,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.cacheType3Record: {0}, {1}, {2}",A,C,B);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
if(l_sObjectKey!==undefined){if(this.m_pObjectDataCacheType3[l_sObjectKey]===undefined){this.m_pObjectDataCacheType3[l_sObjectKey]=new SL_OA();}var l_pFields=C.split(" ");
var l_oRecordFieldData=new SL4B_RecordFieldData();
...
function SL_PY(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.type3Clear({0})",A);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_SD(A,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.deleteType2RecordLevel: {0}; {1}",A,B);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_AR(B,C,A){var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[B];
...
function SL_SC(A,E,C,B,D){var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_QZ(){for(l_sObjectKey in this.m_pContainerCache){var l_oCache=this.m_pContainerCache[l_sObjectKey];
var l_oObjectNameToObjectTypeMap=l_oCache.getObjectNameToObjectTypeMap();
var l_pStructureChanges=[];
var l_pOrderChanges=[];
for(l_sName in l_oObjectNameToObjectTypeMap){this.m_oSubscriptionManager.structureChange(l_sObjectKey,l_sName,l_oObjectNameToObjectTypeMap[l_sName],false,undefined,l_oCache.m_mObjectNameToNumberMap[l_sName]);l_pStructureChanges.push(new SL4B_ContainerStructureChange(l_sName,l_oObjectNameToObjectTypeMap[l_sName],false));}this.m_oSubscriptionManager.structureMultiChange(l_sObjectKey,l_pStructureChanges,l_pOrderChanges,l_oCache.getSize());}}
...
function SL_JK(A,B){var l_sParsedItems=new Array();
...
function SL_RA(A,B,C){var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_OU(A,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.cachePermission({0}, {1})",A,B);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_MC(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.clearPermission: {0}",A);var l_sObjectKey=this.m_pObjectNumberToObjectKeyMap[A];
...
function SL_QP(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectCache.statusUpdated: {0}",A);var l_sObjectNumber=A.getObjectNumber();
...
SL4B_ObjectSubscriptionManager.prototype.requestObject = SL_BS;SL4B_ObjectSubscriptionManager.prototype.requestObjects = SL_ON;SL4B_ObjectSubscriptionManager.prototype.reRequestObjects = SL_BR;SL4B_ObjectSubscriptionManager.prototype.discardObject = SL_DM;SL4B_ObjectSubscriptionManager.prototype.discardObjects = SL_LZ;SL4B_ObjectSubscriptionManager.prototype.addObject = SL_LV;SL4B_ObjectSubscriptionManager.prototype.removeObject = SL_QX;SL4B_ObjectSubscriptionManager.prototype.removeSubscriber = SL_CO;SL4B_ObjectSubscriptionManager.prototype.setObjectCache = SL_AL;SL4B_ObjectSubscriptionManager.prototype.recordMultiUpdated = SL_GJ;SL4B_ObjectSubscriptionManager.prototype.recordMultiUpdated2 = SL_IU;SL4B_ObjectSubscriptionManager.prototype.deleteType2Level = SL_DF;SL4B_ObjectSubscriptionManager.prototype.type2Clear = SL_GM;SL4B_ObjectSubscriptionManager.prototype.type3Clear = SL_DU;SL4B_ObjectSubscriptionManager.prototype.clearAllSubscriptions = SL_JW;SL4B_ObjectSubscriptionManager.prototype.dirUpdated = SL_FE;SL4B_ObjectSubscriptionManager.prototype.dirMultiUpdated = SL_OM;SL4B_ObjectSubscriptionManager.prototype.newsUpdated = SL_PT;SL4B_ObjectSubscriptionManager.prototype.objectInfo = SL_IN;SL4B_ObjectSubscriptionManager.prototype.storyUpdated = SL_BM;SL4B_ObjectSubscriptionManager.prototype.permissionUpdated = SL_NJ;SL4B_ObjectSubscriptionManager.prototype.deleteAllPermissionEntries = SL_FR;SL4B_ObjectSubscriptionManager.prototype.deletePermissionEntry = SL_EA;SL4B_ObjectSubscriptionManager.prototype.objectStatus = SL_MX;SL4B_ObjectSubscriptionManager.prototype.splitFilter = SL_KX;SL4B_ObjectSubscriptionManager.prototype.sendStaleToAll = SL_KC;SL4B_ObjectSubscriptionManager.prototype.sendCurrentObjectStatusToAll = SL_QW;SL4B_ObjectSubscriptionManager.prototype.sendObjectStatusToAll = SL_MO;SL4B_ObjectSubscriptionManager.prototype.structureChange = SL_MH;SL4B_ObjectSubscriptionManager.prototype.structureChange2 = SL_QH;SL4B_ObjectSubscriptionManager.prototype.structureMultiChange = SL_PU;SL4B_ObjectSubscriptionManager.prototype.structureMultiChange2 = SL_CC;SL4B_ObjectSubscriptionManager.prototype.registerProxySubscriber = SL_RP;SL4B_ObjectSubscriptionManager.prototype.getProxySubscriber = SL_IK;SL4B_ObjectSubscriptionManager.prototype.addContainerRequestData = SL_RX;SL4B_ObjectSubscriptionManager.prototype.getContainerRequestData = SL_DI;SL4B_ObjectSubscriptionManager.prototype.getObjectSubscriptions = SL_JM;SL4B_ObjectSubscriptionManager.prototype.getContainerIdToRequestDataMap = SL_EO;SL4B_ObjectSubscriptionManager.prototype.getContainer = function(B,C,E,F,D,A){SL4B_JavaScriptRttpProvider.checkWindowRange(F,D);var l_oListener=SL4B_AbstractRttpProvider.prototype.getListener(B);
...
function SL_RX(F,C,A,D,E,B){var l_oContainerRequestData=new SL4B_ContainerRequestData(F,C,A,D,E,B);
...
function SL_RP(D,C,A,B){var l_oFilterAndFieldList=this.splitFilter(B);
...
function SL_IK(C,A,B){var l_oFilterAndFieldList=this.splitFilter(B);
var l_sObjectKey=A+l_oFilterAndFieldList.m_sFilter;
if(this.m_oContainerProxySubscribers[l_sObjectKey]!=null){return this.m_oContainerProxySubscribers[l_sObjectKey][C];
}return null;
}
function SL_MH(B,D,E,C,F,A){if(typeof (F)!="undefined"){this.structureChange2(B,D,E,C,F,A);}else 
{for(l_sListenerId in this.m_pObjectSubscriptions[B]){this.structureChange2(B,D,E,C,l_sListenerId,A);}}}
function SL_QH(B,D,E,C,F,A){if(this.m_oContainerProxySubscribers[B]!=null){var l_sProxyListenerId=this.m_oContainerProxySubscribers[B][F];
...
function SL_PU(B,A,D,C,E){if(typeof (E)!="undefined"){this.structureMultiChange2(B,A,D,C,E);}else 
{for(l_sListenerId in this.m_pObjectSubscriptions[B]){this.structureMultiChange2(B,A,D,C,l_sListenerId);}}}
function SL_CC(B,A,D,C,E){if(this.m_oContainerProxySubscribers[B]!=null){var l_oSubscription=this.m_pObjectSubscriptions[B][E];
l_oSubscription.m_oSubscriber.WTStructureMultiChange(l_oSubscription.m_sObjectName,A,D,C);}}
function SL_BR(){this.m_bStaleStatusSent=false;SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINE_INT,"ObjectSubscriptionManager.reRequestObjects()");for(l_sObjectKey in this.m_pObjectSubscriptions){var l_pListenerMap=this.m_pObjectSubscriptions[l_sObjectKey];
for(l_oListenerId in l_pListenerMap){var l_oObjectSubscription=l_pListenerMap[l_oListenerId];
if(!l_oObjectSubscription.isProxySubscription()){var sFieldList=l_oObjectSubscription.getFieldMask().getFieldList_deprecated();
this.m_oObjectCache.requestObject(l_oObjectSubscription.m_sObjectName,l_oObjectSubscription.m_sFilter,sFieldList);}}}}
function SL_KC(){if(this.m_bStaleStatusSent===false){this.m_bStaleStatusSent=true;SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINE_INT,"ObjectSubscriptionManager.sendStaleToAll()");this.sendObjectStatusToAll(GF_CachedObjectStatus.const_CONNECTION_LOST);}}
function SL_QW(){this.m_bStaleStatusSent=false;SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINE_INT,"ObjectSubscriptionManager.sendCurrentObjectStatusToAll()");this.sendObjectStatusToAll(null);}
function SL_MO(A){for(l_sObjectKey in this.m_pObjectSubscriptions){var l_pListenerMap=this.m_pObjectSubscriptions[l_sObjectKey];
for(l_oListenerId in l_pListenerMap){var l_oObjectSubscription=l_pListenerMap[l_oListenerId];
...
function SL_KX(A){var l_oReturn=new SL_GT();
...
function SL_BS(D,C,B,A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.requestObject: {0}, {1}, {2}, {3}",D,C,B,A);var l_oFilterAndFieldList=this.splitFilter(B);
...
function SL_ON(C,A,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.requestObjects: {0}, {1}, {2}",C,A,B);var l_oFilterAndFieldList=this.splitFilter(B);
...
function SL_LZ(A,B,C){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.discardObjects: {0}, {1}, {2}",A,B,C);var l_oFilterAndFieldList=this.splitFilter(C);
var l_sValidatedObjectList="";
...
function SL_LV(D,C,B,A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.addObject: {0}, {1}, {2}, {3}",D,C,B,A);var l_bIsValidObject=this.isValidObject(C);
...
function SL_QX(D,C,B,A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.removeObject: {0}, {1}, {2}",D,C,A);if(!this.isValidObject(C)){SL4B_Logger.log(SL4B_DebugLevel.const_WARN_INT,"ObjectSubscriptionManager.removeObject: attempt to remove an invalid object ({0}).",C);return false;
...
function SL_AL(A){this.m_oObjectCache=A;}
...
function SL_IU(D,A,C,B){var l_oSubscription=this.m_pObjectSubscriptions[D][B];
...
function SL_JW(A,B){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.clearAllSubscriptions: {0}",A);for(l_sListenerId in this.m_pObjectSubscriptions[A]){var l_oSubscription=this.m_pObjectSubscriptions[A][l_sListenerId];
...
function SL_OM(A,B){for(l_sListenerId in this.m_pObjectSubscriptions[A]){var l_oSubscription=this.m_pObjectSubscriptions[A][l_sListenerId];
l_oSubscription.m_oSubscriber.WTDirMultiUpdated(l_oSubscription.m_sObjectName,B);}}
function SL_PT(A,D,C,B){for(l_sListenerId in this.m_pObjectSubscriptions[A]){var l_oSubscription=this.m_pObjectSubscriptions[A][l_sListenerId];
l_oSubscription.m_oSubscriber.WTNewsUpdated(l_oSubscription.m_sObjectName,D,C,B);}}
function SL_IN(B,C,D,A){for(l_sListenerId in this.m_pObjectSubscriptions[B]){var l_oSubscription=this.m_pObjectSubscriptions[B][l_sListenerId];
l_oSubscription.m_oSubscriber.WTObjectInfo(l_oSubscription.m_sObjectName,C,D,A);}}
function SL_BM(B,A){for(l_sListenerId in this.m_pObjectSubscriptions[B]){var l_oSubscription=this.m_pObjectSubscriptions[B][l_sListenerId];
for(var i=0;i<A.length;i++){if(i||A[i]){l_oSubscription.m_oSubscriber.WTStoryUpdated(B,A[i]||"\\n");}}}}
function SL_NJ(A,B,C){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscriptionManager.permissionUpdated: {0}, {1}, {2}",A,B,C);for(l_sListenerId in this.m_pObjectSubscriptions[A]){var l_oSubscription=this.m_pObjectSubscriptions[A][l_sListenerId];
var l_bAllFields=l_oSubscription.isSubscribedToAllFields();
...
function SL_MX(B,C,A,D){for(l_sListenerId in this.m_pObjectSubscriptions[B]){this.objectStatusForListener(B,C,A,D,l_sListenerId);}}
...
function SL_JM(){return this.m_pObjectSubscriptions;
}
...
SL4B_ActionSubscriptionManager=GF_ActionSubscriptionManager;GF_ActionSubscriptionManager.prototype.sendActionMessage = SL_KV;GF_ActionSubscriptionManager.prototype.contribObject = SL_RY;GF_ActionSubscriptionManager.prototype.createObject = SL_LH;GF_ActionSubscriptionManager.prototype.deleteObject = SL_RO;GF_ActionSubscriptionManager.prototype.throttleObjects = SL_MJ;GF_ActionSubscriptionManager.prototype.throttleGlobal = SL_KM;GF_ActionSubscriptionManager.NULL_SUBSCRIBER=new SL4B_AbstractSubscriber();GF_ActionSubscriptionManager.NULL_SUBSCRIBER.ID="SL4B_ActionSubscriptionManager.NULL_SUBSCRIBER";GF_ActionSubscriptionManager.NULL_SUBSCRIBER.contribOk = function(){};
...
function SL_KV(A,B){SL4B_Accessor.getUnderlyingRttpProvider().getManagedConnection().sendMessage(B,A);}
...
function SL_RY(B,C,D,A){var l_nLength=D.size();
...
pContribMessage[2+l_nField]=" "+GF_RequestEncoder.encodeRttpData(l_oField.m_sName)+"="+GF_RequestEncoder.encodeRttpData(l_oField.m_sValue);}var l_oActionSubscription=new SL_BZ(B,C,D);
...
function SL_LH(D,B,C,A){if(this._isInvocationWithoutSubscriberArugment(D)){return this._createObject(GF_ActionSubscriptionManager.NULL_SUBSCRIBER.ID,D,B,C);
}else 
...
var oActionSubscription=new SL_MR(C,A,D);
...
function SL_RO(C,B,A){if(this._isInvocationWithoutSubscriberArugment(C)){return this._deleteObject(GF_ActionSubscriptionManager.NULL_SUBSCRIBER.ID,C,B);
}else 
...
var oActionSubscription=new SL_KF(C,A);
...
function SL_MJ(A,B){if(SL4B_ThrottleLevel.isValid(B)===false){throw new SL4B_Exception("'"+B+"' is not a valid throttle level.");
}var l_pObjectNames=A.split(SL4B_AbstractRttpProvider.const_OBJECT_NAME_DELIMITER);
...
function SL_KM(A){if(SL4B_ThrottleLevel.isValid(A)===false){throw new SL4B_Exception("'"+A+"' is not a valid throttle level.");
...
function SL_NL(A){this.m_oSubscriber=this._getSubscriberFromId(A);this.m_oPersistentActionKey=null;}
SL_NL.prototype._getSubscriberFromId = function(A){if(A){return eval(A);
}else 
{return GF_ActionSubscriptionManager.NULL_SUBSCRIBER;
}};
SL_NL.prototype.receiveMessage = function(A,B){throw new SL4B_Error("AbstractActionSubscription.receiveMessage: method not implemented");
};
SL_NL.prototype._setPersistentActionKey = function(A){this.m_oPersistentActionKey=A;};
SL_NL.prototype.getFailureDescription = function(A){var l_sDescription;
switch(A){
...
SL_NL.prototype.getAuthCode = function(A){var l_nAuthCode;
...
function SL_BZ(C,A,B){SL_NL.call(this,C);this.m_sObjectName=A;this.m_oFieldData=B;}
SL_BZ.prototype = new SL_NL;SL_BZ.prototype.receiveMessage = function(B,A){var l_nRttpCode=B.getRttpCode();
switch(l_nRttpCode){
...
function SL_MR(C,A,B){SL_NL.call(this,C);this.m_sObjectName=A;this.m_sRttpType=B;}
SL_MR.prototype = new SL_NL;SL_MR.prototype.receiveMessage = function(B,A){var l_nRttpCode=B.getRttpCode();
switch(l_nRttpCode){
...
SL_DC.prototype = new SL_NL;SL_DC.prototype.receiveMessage = function(B,A){var l_nRttpCode=B.getRttpCode();
switch(l_nRttpCode){
case SL4B_RttpCodes.const_THROTTLE_OK:break;
default :break;
}};
function SL_KF(B,A){SL_NL.call(this,B);this.m_sObjectName=A;}
SL_KF.prototype = new SL_NL;SL_KF.prototype.receiveMessage = function(B,A){var l_nRttpCode=B.getRttpCode();
switch(l_nRttpCode){
...
SL4B_ObjectSubscription.prototype.addFields = SL_AP;SL4B_ObjectSubscription.prototype.removeFields = SL_BJ;SL4B_ObjectSubscription.prototype.isSubscribedToField = SL_MM;SL4B_ObjectSubscription.prototype.isSubscribedToAllFields = function(){return this.m_oFieldMask.isSubscribedToAll();
};
...
SL4B_ObjectSubscription.prototype.getSubscriptionId = SL_AN;SL4B_ObjectSubscription.prototype.getFilter = SL_CY;SL4B_ObjectSubscription.prototype.getSubscriber = SL_QU;SL4B_ObjectSubscription.prototype.getObjectName = SL_LG;function SL_AP(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscription.addFields: {0}",A);if(SL4B_ObjectSubscription.isAllFields(A)){this.m_oFieldMask.addAll();}else 
{var pFields=A.split(",");
for(var i=0;i<pFields.length;++i){var sField=pFields[i];
this.m_oFieldMask.add(sField);}}}
function SL_BJ(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"ObjectSubscription.removeFields: {0}",A);var bChanged=false;
...
function SL_MM(A){return this.m_oFieldMask.contains(A);
}
...
GF_CachedObjectStatus.const_CONNECTION_LOST=new GF_CachedObjectStatus(SL4B_ObjectStatus.STALE,0,"Liberator connection lost");GF_CachedObjectStatus.const_SUBSCRIBING=new GF_CachedObjectStatus(SL4B_ObjectStatus.STALE,0,"Subscribing to object");function SL_AN(){return this.m_sSubscriptionId;
}
function SL_CY(){return this.m_sFilter;
}
function SL_QU(){return this.m_oSubscriber;
}
function SL_LG(){return this.m_sObjectName;
}
...
function SL_NG(B,A){if(A>=B.length){throw new SL4B_Exception("An illegal message index ("+A+") was received");
...
SL4B_Type2RecordCache.prototype.addField = SL_ND;SL4B_Type2RecordCache.prototype.getField = SL_LX;SL4B_Type2RecordCache.prototype.deleteLevel = SL_HK;SL4B_Type2RecordCache.prototype.dump = SL_JX;function SL_ND(B,C,A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"Type2RecordCache.addField: {0}, {1}, {2}",B,C,A);if(typeof this.m_pLevelCache[B]=="undefined"){this.m_pLevelCache[B]=new Object();}this.m_pLevelCache[B][C]=A;}
...
function SL_JX(){for(l_sLevel in this.m_pLevelCache){var l_oFields=this.m_pLevelCache[l_sLevel];
...
function SL_LX(A,B){var l_oFieldCache=this.m_pLevelCache[A];
var l_sValue=null;
...
function SL_OA(){this.m_pLevelCache=new Array();}
SL_OA.prototype.getField = SL_IE;SL_OA.prototype.dump = SL_BO;function SL_IE(A,B){var l_oFieldCache=this.m_pLevelCache[A];
var l_sValue=null;
if(typeof l_oFieldCache=="undefined"){l_sValue=null;}else 
{l_sValue=l_oFieldCache[B];if(typeof l_sValue=="undefined"){l_sValue=null;}}return l_sValue;
}
function SL_BO(){for(l_sLevel in this.m_pLevelCache){var l_oFields=this.m_pLevelCache[l_sLevel];
SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"Level: {0}",l_sLevel);for(l_sFieldName in l_oFields){var l_sValue=l_oFields[l_sFieldName];
SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"\t{0}={1}",l_sFieldName,l_sValue);}}}
...
SL4B_NewsHeadlineCache.m_HEADLINE_CACHE_LIMIT=200;SL4B_NewsHeadlineCache.const_SIZE_FIELD="size";SL4B_NewsHeadlineCache.prototype.addHeadline = function(C,B,A){if(this.m_pHeadlines.length>=SL4B_NewsHeadlineCache.m_HEADLINE_CACHE_LIMIT){this.m_pHeadlines.shift();}this.m_pHeadlines.push(new SL_HM(C,B,A));};
...
function SL_HM(C,B,A){this.m_sStoryCode=C;this.m_sHeadline=B;this.m_sDate=A;}
...
SL4B_BaseStreamingConnection.prototype = new SL4B_AbstractConnection;SL4B_BaseStreamingConnection.prototype.connect = SL_KJ;SL4B_BaseStreamingConnection.prototype.getScriptPathUrl = SL_JN;SL4B_BaseStreamingConnection.prototype.reconnect = SL_LU;SL4B_BaseStreamingConnection.prototype.setResponseHttpRequest = SL_KQ;SL4B_BaseStreamingConnection.prototype.start = SL_GE;SL4B_BaseStreamingConnection.prototype.super_stop = SL4B_BaseStreamingConnection.prototype.stop;SL4B_BaseStreamingConnection.prototype.stop = SL_FK;SL4B_BaseStreamingConnection.prototype.super_parseRttpMessage = SL4B_BaseStreamingConnection.prototype.parseRttpMessage;SL4B_BaseStreamingConnection.prototype.parseRttpMessage = SL_AH;SL4B_BaseStreamingConnection.prototype.super_processRttpMessageBlock = SL4B_AbstractConnection.prototype.processRttpMessageBlock;SL4B_BaseStreamingConnection.RTTP_MESSAGE_REG_EXP=/^[0-9A-Za-z-_]{2,2}([0-9A-Za-z-_]{2,2})[0-9A-Za-z-_]{4,4}/;function SL_KJ(){SL4B_Logger.logConnectionMessage(false,"BaseStreamingConnection.connect");this.m_nMessageCount=0;if(this.m_nConnectedState==0&&this.m_oRequestHttpRequest!=null){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINEST_INT,"  connecting...");var l_sResponseFrameUrl;
...
function SL_JN(A,B){var l_sRootUrl=SL4B_ScriptLoader.getRootUrl();
...
function SL_AH(A){if(this.m_nConnectedState==2){this.super_parseRttpMessage(A);this.m_nMessageCount++;}else 
...
function SL_LU(){SL4B_Logger.log(SL4B_DebugLevel.const_INFO_INT,"BaseStreamingConnection.reconnect()");var oConnectionManager=this.m_oRttpProvider.m_oConnectionManager;
...
function SL_KQ(A){SL4B_Logger.log(SL4B_DebugLevel.const_RTTP_FINE_INT,"BaseStreamingConnection.setResponseHttpRequest");this.m_oResponseHttpRequest=A;if(this.m_nConnectedState==1){this.m_nConnectedState=2;}}
...
function SL_RD(A){this.clearPollTimeout();this.super_send(A);}
function SL_NM(A){this.m_oRequestHttpRequest=A;this.clearPollTimeout();this.m_nPollTimeout=setTimeout("SL4B_ConnectionProxy.getInstance().getConnection().pollServer()",this.m_nRefreshPeriod);}
function SL_DY(){if(this.m_nPollTimeout!==null){clearTimeout(this.m_nPollTimeout);this.m_nPollTimeout=null;}}
function SL_RG(){if(this.m_nPollTimeout!=null&&this.m_oRttpProvider.getSessionId()!=null){this.m_oRttpProvider.getManagedConnection()._sendPriorityMessage("",SL4B_ManagedConnection.NO_RESPONSE_EXPECTED_MESSAGE_RECEIVER,true);}}
function SL_NV(){SL4B_Logger.logConnectionMessage(false,"Type3Connection.start");this.createStandardRequestChannel();}
function SL_ML(){this.super_stop();this.clearPollTimeout();}
...
SL4B_JavaScriptRttpProvider.prototype.receiveMessage = SL_PA;SL4B_JavaScriptRttpProvider.prototype.initialise = SL_HV;SL4B_JavaScriptRttpProvider.prototype.onLoad = SL_CU;SL4B_JavaScriptRttpProvider.prototype.responseHttpRequestReady = SL_CE;SL4B_JavaScriptRttpProvider.prototype.requestHttpRequestReady = SL_BA;SL4B_JavaScriptRttpProvider.prototype.createConnection = SL_GI;SL4B_JavaScriptRttpProvider.prototype.createConnectionLost = SL_FF;SL4B_JavaScriptRttpProvider.prototype.connect = SL_OX;SL4B_JavaScriptRttpProvider.prototype.login = SL_OC;SL4B_JavaScriptRttpProvider.prototype.logout = SL_RC;SL4B_JavaScriptRttpProvider.prototype.super_stop = SL4B_JavaScriptRttpProvider.prototype.stop;SL4B_JavaScriptRttpProvider.prototype.stop = SL_BB;SL4B_JavaScriptRttpProvider.prototype.getVersion = SL_ES;SL4B_JavaScriptRttpProvider.prototype.getVersionInfo = SL_QT;SL4B_JavaScriptRttpProvider.prototype.getObject = SL_LS;SL4B_JavaScriptRttpProvider.prototype.getObjects = SL_LL;SL4B_JavaScriptRttpProvider.prototype.removeObject = SL_LQ;SL4B_JavaScriptRttpProvider.prototype.removeObjects = SL_CR;SL4B_JavaScriptRttpProvider.prototype.removeSubscriber = SL_JI;SL4B_JavaScriptRttpProvider.prototype.contribObject = SL_RH;SL4B_JavaScriptRttpProvider.prototype.createObject = SL_HB;SL4B_JavaScriptRttpProvider.prototype.deleteObject = SL_KH;SL4B_JavaScriptRttpProvider.prototype.reconnect = SL_MZ;SL4B_JavaScriptRttpProvider.prototype.checkConnected = SL_FU;SL4B_JavaScriptRttpProvider.prototype.getJsContainerUrl = SL_JS;SL4B_JavaScriptRttpProvider.prototype.getObjectCache = SL_RZ;SL4B_JavaScriptRttpProvider.prototype.getSubscriptionManager = SL_BT;SL4B_JavaScriptRttpProvider.prototype.getAutoDirectory = SL_NX;SL4B_JavaScriptRttpProvider.prototype.removeAutoDirectory = SL_FW;SL4B_JavaScriptRttpProvider.prototype.getContainer = SL_EK;SL4B_JavaScriptRttpProvider.prototype._$getContainerSnapshot = SL_GY;SL4B_JavaScriptRttpProvider.prototype.setContainerWindow = SL_ED;SL4B_JavaScriptRttpProvider.prototype.clearContainerWindow = SL_NK;SL4B_JavaScriptRttpProvider.prototype.removeContainer = SL_OK;SL4B_JavaScriptRttpProvider.prototype.getFieldName = SL_CA;SL4B_JavaScriptRttpProvider.prototype.getFieldNames = function(){return "";
};
SL4B_JavaScriptRttpProvider.prototype.setGlobalThrottle = SL_IT;SL4B_JavaScriptRttpProvider.prototype.setThrottleObject = SL_AY;SL4B_JavaScriptRttpProvider.prototype.setThrottleObjects = SL_AF;SL4B_JavaScriptRttpProvider.prototype.addConnectionListener = function(A){this.m_oConnectionManager.addConnectionListener(A);};
...
function SL_PA(A,C,B){if(SL4B_Accessor.getRttpProvider()!=this&&SL4B_Accessor.getUnderlyingRttpProvider()!=this){SL4B_Logger.log(SL4B_DebugLevel.const_WARN_INT,"JavaScriptRttpProvider: Received a message when not the current RttpProvider. {0}",A);return;
...
function SL_HV(){SL4B_Logger.logConnectionMessage(false,"JavaScriptRttpProvider.initialise");this.m_oConnectionManager.setMessageReceiver(this.m_oManagedConnection);this.m_bFatalError=(this.m_oConnectionManager.initialise()==false);}
function SL_JS(){return this.m_oConnectionManager.getJsContainerUrl();
}
function SL_RZ(){return this.m_oObjectCache;
}
function SL_BT(){return this.m_oObjectSubscriptionManager;
}
function SL_CE(){this.m_oConnectionManager.responseHttpRequestReady();}
function SL_BA(){this.m_oConnectionManager.requestHttpRequestReady();}
...
function SL_OX(){this.m_oConnectionManager.connect();}
function SL_MZ(){this.m_oConnectionManager.reconnect();}
function SL_OC(A,B){this.m_oConnectionManager.login(A,B);}
function SL_IT(A){this.m_oActionSubscriptionManager.throttleGlobal(A);}
function SL_AF(A,B){this.m_oActionSubscriptionManager.throttleObjects(A,B);}
function SL_AY(A,B){this.m_oActionSubscriptionManager.throttleObjects(A,B);}
function SL_RC(){this.m_oConnectionManager.logout();}
function SL_BB(){this.m_oConnectionManager.stop();}
function SL_ES(){return SL4B_Version.getVersion();
}
function SL_QT(){return SL4B_Version.getVersionInfo();
}
function SL_LS(C,B,A){this.m_oObjectSubscriptionManager.requestObject(this.getListener(C),B,A);}
function SL_LL(C,A,B){this.m_oObjectSubscriptionManager.requestObjects(this.getListener(C),A,B);}
function SL_LQ(C,B,A){this.m_oObjectSubscriptionManager.discardObject(this.getListener(C),B,A);}
function SL_CR(C,A,B){this.m_oObjectSubscriptionManager.discardObjects(this.getListener(C),A,B);}
function SL_JI(A){this.m_oObjectSubscriptionManager.removeSubscriber(this.getListener(A));}
...
function SL_RH(D,B,C,A){if(C==null||C.size()==0){throw new SL4B_Exception("JavaScriptRttpProvider.contribObject: field data object cannot be null or empty");
...
function SL_KH(C,B,A){return this.m_oActionSubscriptionManager.deleteObject(this.getListener(C),B,A);
}
function SL_NX(E,D,A,B,C){var l_sCombinedFieldList=this.createFieldListForAutoDirectory(B,C,A);
...
function SL_NK(A){this.setContainerWindow(A);}
function SL_OK(B,A){var l_oRequestData=this.m_oObjectSubscriptionManager.getContainerRequestData(A);
...
function SL_CA(A){return this.m_oConnectionManager.getFieldName(A);
}
...
SL4B_TestRttpProvider.prototype = new SL4B_AbstractRttpProvider;SL4B_TestRttpProvider.prototype.tick = SL_CT;SL4B_TestRttpProvider.prototype.addDecimalField = SL_CV;SL4B_TestRttpProvider.prototype.addIntegerField = SL_JA;SL4B_TestRttpProvider.prototype.addStringField = SL_BQ;SL4B_TestRttpProvider.prototype.addObject = SL_EQ;SL4B_TestRttpProvider.prototype.getNextValue = SL_LM;SL4B_TestRttpProvider.prototype.initialise = SL_GA;SL4B_TestRttpProvider.prototype.connect = SL_EY;SL4B_TestRttpProvider.prototype.login = SL_FQ;SL4B_TestRttpProvider.prototype.getObject = SL_DJ;SL4B_TestRttpProvider.prototype.getObjects = SL_PF;SL4B_TestRttpProvider.prototype.removeObject = SL_FS;SL4B_TestRttpProvider.prototype.removeObjects = SL_EF;SL4B_TestRttpProvider.prototype.getObjectType = SL_MQ;SL4B_TestRttpProvider.prototype.setThrottleObject = SL_LP;SL4B_TestRttpProvider.prototype.setThrottleObjects = SL_RR;SL4B_TestRttpProvider.prototype.setGlobalThrottle = SL_QS;SL4B_TestRttpProvider.prototype.disableWTStatsTimeout = SL_RK;SL4B_TestRttpProvider.prototype.clearObjectListeners = SL_KA;SL4B_TestRttpProvider.prototype.blockObjectListeners = SL_HF;SL4B_TestRttpProvider.prototype.unblockObjectListeners = SL_LO;SL4B_TestRttpProvider.prototype.createObject = SL_OE;SL4B_TestRttpProvider.prototype.contribObject = SL_AU;SL4B_TestRttpProvider.prototype.deleteObject = SL_AT;SL4B_TestRttpProvider.prototype.getFieldNames = SL_BP;SL4B_TestRttpProvider.prototype.logout = SL_QD;SL4B_TestRttpProvider.prototype.debug = SL_JB;SL4B_TestRttpProvider.prototype.setDebugLevel = SL_OG;SL4B_TestRttpProvider.prototype.getVersion = SL_MI;SL4B_TestRttpProvider.prototype.getVersionInfo = SL_PI;function SL_EY(){setTimeout("SL4B_Accessor.getRttpProvider().connected()",100);}
...
if(typeof l_oLiberatorSubscription=="undefined"){l_oLiberatorSubscription=new SL_IZ();l_oLiberatorSubscription.m_oSubscriber=C;this.m_pSubscriptions[this.getListener(C)]=l_oLiberatorSubscription;}var l_oSubscriptionObject=l_oLiberatorSubscription.m_pObjects[B];
...
function SL_PF(C,A,B){l_pObjects=A.split(" ");for(var l_nCount=0;l_nCount<l_pObjects.length;l_nCount++){var l_sObjectName=l_pObjects[l_nCount];
...
function SL_MQ(B,A){throw new SL4B_Error("getObjectType method not implemented in TestRttpProvider");
}
function SL_LP(A,B){var l_oObject=this.m_pObjects[A];
if(typeof l_oObject!="undefined"){}throw new SL4B_Error("setThrottleObject method not implemented in TestRttpProvider");
}
function SL_RR(A,B){throw new SL4B_Error("setThrottleObjects method not implemented in TestRttpProvider");
}
function SL_QS(A){throw new SL4B_Error("setGlobalThrottle method not implemented in TestRttpProvider");
}
function SL_RK(A){throw new SL4B_Error("disableWTStatsTimeout method not implemented in TestRttpProvider");
}
function SL_KA(B,A){if(typeof A=="undefined"){}else 
{}}
function SL_HF(B,A){if(typeof A=="undefined"){}else 
{}}
function SL_LO(B,A){if(typeof A=="undefined"){}else 
{}}
function SL_OE(C,A,B){throw new SL4B_Error("createObject method not implemented in TestRttpProvider");
}
function SL_AU(C,A,B){for(var l_nFieldIndex=0,l_nLength=B.size();l_nFieldIndex<l_nLength;++l_nFieldIndex){var l_oField=B.getField(l_nFieldIndex);
if(l_nFieldIndex==(l_nLength-1)){this.m_oRttpApplet.contribObject(A,l_oField.m_sName,l_oField.m_sValue,this.getListener(C));}else 
{this.m_oRttpApplet.contribObject(A,l_oField.m_sName,l_oField.m_sValue,null);}}}
function SL_AT(B,A){throw new SL4B_Error("deleteObject method not implemented in TestRttpProvider");
}
function SL_BP(){var l_sNames="";
...
function SL_QD(){clearInterval(this.m_hSetInterval);}
function SL_JB(B,A){throw new SL4B_Error("debug method not implemented in TestRttpProvider");
}
function SL_OG(A){throw new SL4B_Error("setDebugLevel method not implemented in TestRttpProvider");
}
function SL_MI(){throw new SL4B_Error("getVersion method not implemented in TestRttpProvider");
}
function SL_PI(){throw new SL4B_Error("getVersionInfo method not implemented in TestRttpProvider");
}
function SL_JY(){this.m_sName="";this.m_sType="numeric";this.m_sDefaultValue="?";this.m_nMax;this.m_nMin;}
...
function SL_CB(){this.m_sName="";this.m_sValue="";this.m_nUpdateId=0;}
function SL_IZ(){this.m_oSubscriber=null;this.m_pObjects=new Object();this.toString = function(){var l_sRtn="";
...
function SL_ID(){this.m_sName=null;this.m_sValue=null;}
...
function SL_LM(B,A){var l_oReturn="";
...
if(typeof l_oFieldValue!="undefined"&&l_oFieldValue.m_nUpdateId>l_oLiberatorSubsciptionObject.m_nUpdateId){var l_oNotificationFieldValue=new SL_ID();
...
function SL_LD(){this.m_nSliceSize=20;this.m_pRtmlObjectContainers=new Object();this.initialise();}
SL_LD.prototype = new SL4B_AbstractSubscriber;SL_LD.prototype.ready = SL_KN;SL_LD.prototype.getRequiredSymbols = SL_MD;SL_LD.prototype.optimiseAndRequestObjects = SL_KD;SL_LD.prototype.recordUpdated = SL_BD;SL_LD.prototype.objectStatus = SL_OW;function SL_KN(){SL4B_Logger.log(SL4B_DebugLevel.const_DEBUG_INT,"RtmlSubscriber.ready: called.");var l_pElements=this.getRequiredSymbols();
...
function SL_MD(){SL4B_Logger.log(SL4B_DebugLevel.const_INFO_INT,"RtmlSubscriber: Finding quote tags...");var l_oQuoteTag=null;
var l_nCount=1;
...
function SL_KD(A){for(var l_i=0;l_i<A.length;l_i++){var l_oQuoteElement=new SL_AE(A[l_i]);
...
for(l_sObjectName in this.m_pRtmlObjectContainers){l_pTagList=l_pTagList.concat(this.m_pRtmlObjectContainers[l_sObjectName].getUniqueFields());}l_pTagList.sort(SL_PO);for(var l_i=l_pTagList.length-1;l_i>0;l_i--){if((l_pTagList[l_i].m_sFieldName==l_pTagList[l_i-1].m_sFieldName)){l_pTagList[l_i-1].m_sObjectName+=SL4B_AbstractRttpProvider.const_OBJECT_NAME_DELIMITER+l_pTagList[l_i].m_sObjectName;l_pTagList=l_pTagList.slice(0,l_i).concat(l_pTagList.slice(l_i+1));}}l_pTagList.sort(SL_RL);for(var l_i=l_pTagList.length-1;l_i>0;l_i--){if((l_pTagList[l_i].m_sObjectName==l_pTagList[l_i-1].m_sObjectName)){l_pTagList[l_i-1].m_sFieldName+=","+l_pTagList[l_i].m_sFieldName;l_pTagList=l_pTagList.slice(0,l_i).concat(l_pTagList.slice(l_i+1));}}var l_nRequests=0;
...
function SL_PO(A,B){if(A.m_sFieldName>B.m_sFieldName){return 1;
}if(A.m_sFieldName<B.m_sFieldName){return -1;
}if(A.m_sObjectName>B.m_sObjectName){return 1;
}if(A.m_sObjectName<B.m_sObjectName){return -1;
}return 0;
}
function SL_RL(A,B){if(A.m_sObjectName>B.m_sObjectName){return 1;
}if(A.m_sObjectName<B.m_sObjectName){return -1;
}return 0;
}
function SL_BD(B,C,A){this.m_pRtmlObjectContainers[B].updateRtmlQuoteElements(C,A);}
function SL_OW(D,B,A,C){if(B==SL4B_ObjectStatus.STALE){this.m_pRtmlObjectContainers[D].updateRtmlQuoteElementsStatus(true);}else 
...
SL_AE.prototype.updateQuoteValue = SL_PC;SL_AE.prototype.updateQuoteStatus = SL_QY;SL_AE.prototype.getQuoteAttribute = SL_KB;SL_AE.prototype.getParentAttribute = SL_MV;SL_AE.const_INTEGER=1;SL_AE.const_POSITIVE_INTEGER=2;SL_AE.const_STRING=3;SL_AE.const_FGCOLOR=4;SL_AE.const_BGCOLOR=5;function RTML_RelativeUpdate(A,C,D,G,B,E,F){var l_oUpdateFlash=SL_DB(A,C,D.value,G,B);
...
g_pPreviousValue[B]=C.value;var l_oFinalUpdate=SL_NT(D,0,GF_ConvertToDecimal(C.value),'0.00');
...
var l_oRelUpdate=SL_NT(D,l_nOld,l_nNew,l_sPrevVal);
var l_oAbsUpdate=SL_NT(D,0,l_nNew,'0.00');
...
function SL_PC(A){if(!this.m_bIsIndicator){var l_dNewComparisonValue=A;
...
function SL_QY(A){if(!this.m_bIsIndicator){if(A){RTSL_FlushUpdate(this.m_oHtmlElement.getAttribute("id"));this.m_oHtmlElement.style.textDecoration="line-through";}else 
{this.m_oHtmlElement.style.textDecoration="none";}}}
function SL_KB(B,A){var isError=false;
...
function SL_MV(A){var l_vAttributeValue=null;
...
SL_CJ.prototype.addRtmlQuoteElement = SL_JT;SL_CJ.prototype.getUniqueFields = SL_LE;SL_CJ.prototype.updateRtmlQuoteElements = SL_FY;SL_CJ.prototype.updateRtmlQuoteElementsStatus = SL_GP;function SL_JT(A){if(this.m_pRtmlQuoteElements[A.m_sFieldName]==null){this.m_pRtmlQuoteElements[A.m_sFieldName]=new Array();}this.m_pRtmlQuoteElements[A.m_sFieldName].push(A);}
function SL_LE(){var l_pUniqueFields=new Array();
...
RTML_RtmlSubscriberAccessor = new function(){this.m_oRtmlSubscriber=new SL_LD();this.getRtmlSubscriber = function(A){return ((typeof A=="undefined") ? this.m_oRtmlSubscriber : A);
};
};
...
function RTSL_FractionToDecimal(A){if(!SL_MG(A)){return A;
...
function RTSL_FractionSimplify(A){if(!SL_MG(A)){return A;
}var l_nInt=SL_CZ(A);
if(isNaN(l_nInt)){return A;
}var l_nNum=SL_LI(A);
if(isNaN(l_nNum)){return A;
...
}var l_nHcm=SL_RT(l_nNum,l_nDen);
...
A=l_sInt+SL_KZ(l_sFraction);}else 
if(l_sMatch=A.match(/[0-9]+\/[0-9]+$/)){var l_sFraction=(l_sMatch.toString().match(/[0-9]+\/[0-9]+$/)).toString();
A=SL_KZ(l_sFraction);if(A.charAt(0)==0){A=A.substring(1);}}return A;
}
function SL_KZ(A){var l_sResult;
var l_nFrac=eval(A);
switch(l_nFrac){
...
l_sEnd=SL_IQ(l_sEnd,A);if(l_sBegin.length){if(l_sEnd.length<A){for(var l_nCount=l_sEnd.length;l_nCount<A;l_nCount++){l_sEnd+="0";}}else 
if(l_sEnd.length>A){l_sEnd=l_sEnd.substring(0,A);}}return ((l_bSign) ? ("-") : (""))+l_sBegin+l_sEnd;
}
function SL_IQ(B,A){var l_bPadZeros=true;
...
function SL_MG(A){return (A.toString().indexOf("/",1)==-1) ? false : true;
}
...
function SL_HR(){}
SL_HR.prototype = new SL4B_ConnectionListener;SL_HR.prototype.loginOk = SL_SA;SL_HR.prototype.loginError = SL_QM;SL_HR.prototype.connectionOk = SL_AX;SL_HR.prototype.connectionError = SL_IW;function SL_SA(){if(typeof RTSL_LoginOk=="function"){RTSL_LoginOk(null);}}
function SL_QM(A){if(typeof RTSL_LoginFailed=="function"){RTSL_LoginFailed(null,A);}else 
...
function SL_AX(C,A,B){if(typeof RTSL_StatusConnected=="function"){RTSL_StatusConnected(null);}}
function SL_IW(){if(typeof RTSL_ConnectionLost=="function"){RTSL_ConnectionLost(null);}else 
{SL4B_Logger.synchronizedAlert("Connection lost - reconnection failed");}}
function SL_IX(){SL4B_Accessor.getRttpProvider().addConnectionListener(new SL_HR());}
function SL_EW(){this.initialise();}
SL_EW.prototype = new SL4B_AbstractSubscriber;SL_EW.prototype.ready = SL_CK;SL_EW.prototype.directoryUpdated = SL_KL;SL_EW.prototype.newsUpdated = SL_KU;SL_EW.prototype.objectUpdated = SL_PH;SL_EW.prototype.pageUpdated = SL_BF;SL_EW.prototype.super_recordMultiUpdated = SL_EW.prototype.recordMultiUpdated;SL_EW.prototype.recordMultiUpdated = SL_OJ;SL_EW.prototype.recordUpdated = SL_PX;SL_EW.prototype.storyReset = SL_BG;SL_EW.prototype.storyUpdated = SL_OH;SL_EW.prototype.chat = SL_CD;SL_EW.prototype.contribFailed = SL_QC;SL_EW.prototype.contribOk = SL_JZ;SL_EW.prototype.fieldDeleted = SL_HC;SL_EW.prototype.type2Clear = SL_NI;SL_EW.prototype.type3Clear = SL_PQ;SL_EW.prototype.objectDeleted = SL_ET;SL_EW.prototype.objectInfo = SL_EL;SL_EW.prototype.objectNotFound = SL_FJ;SL_EW.prototype.objectStatus = SL_KS;SL_EW.prototype.objectType = SL_JQ;SL_EW.prototype.objectUnavailable = SL_IY;SL_EW.prototype.objectNotStale = SL_NR;SL_EW.prototype.objectStale = SL_IC;SL_EW.prototype.objectReadDenied = SL_NF;SL_EW.prototype.objectWriteDenied = SL_IB;SL_EW.prototype.clientMethodUndefined = SL_IO;function SL_CK(){if(typeof RTSL_Ready=="function"){RTSL_Ready();}else 
...
function SL_KL(D,C,A,B){if(typeof RTSL_DirUpdated=="function"){RTSL_DirUpdated(D,C,A,B);}else 
{this.clientMethodUndefined("RTSL_DirUpdated");}}
function SL_KU(C,D,B,A){if(typeof RTSL_NewsUpdated=="function"){RTSL_NewsUpdated(C,D,B,A);}else 
{this.clientMethodUndefined("RTSL_NewsUpdated");}}
function SL_PH(A){if(typeof RTSL_ObjectUpdated=="function"){RTSL_ObjectUpdated(A);}else 
{this.clientMethodUndefined("RTSL_ObjectUpdated");}}
function SL_BF(D,B,A,E,C){if(typeof RTSL_PageUpdated=="function"){RTSL_PageUpdated(D,B,A,E,C);}else 
{this.clientMethodUndefined("RTSL_PageUpdated");}}
function SL_OJ(A,B){if(typeof RTSL_RecordMultiUpdated=="function"){RTSL_RecordMultiUpdated(A,B);}else 
{this.super_recordMultiUpdated(A,B);}}
function SL_PX(B,C,A){if(typeof RTSL_RecordUpdated=="function"){RTSL_RecordUpdated(B,C,A);}else 
{this.clientMethodUndefined("RTSL_RecordUpdated");}}
function SL_BG(A){if(typeof RTSL_StoryReset=="function"){RTSL_StoryReset(A);}else 
{this.clientMethodUndefined("RTSL_StoryReset");}}
function SL_OH(B,A){if(typeof RTSL_StoryUpdated=="function"){RTSL_StoryUpdated(B,A);}else 
{this.clientMethodUndefined("RTSL_StoryUpdated");}}
function SL_CD(){}
function SL_QC(B,A,C){if(typeof RTSL_ContribFailed=="function"){RTSL_ContribFailed(B,A,C);}else 
{this.clientMethodUndefined("RTSL_ContribFailed");}}
function SL_JZ(B,A){if(typeof RTSL_ContribOk=="function"){RTSL_ContribOk(B,A);}else 
...
function SL_NI(A){if(typeof RTSL_Type2Clear=="function"){RTSL_Type2Clear(A);}else 
{this.clientMethodUndefined("RTSL_Type2Clear");}}
function SL_PQ(A){if(typeof RTSL_Type3Clear=="function"){RTSL_Type3Clear(A);}else 
...
function SL_KS(C,B,A,D){if(typeof RTSL_ObjectStatus=="function"){RTSL_ObjectStatus(C,B,A,D);}else 
{this.clientMethodUndefined("RTSL_ObjectStatus");}}
function SL_JQ(C,B,A){if(typeof RTSL_ObjectType=="function"){RTSL_ObjectType(C,B,A);}else 
{this.clientMethodUndefined("RTSL_ObjectType");}}
function SL_IY(A){if(typeof RTSL_ObjectUnavailable=="function"){RTSL_ObjectUnavailable(A);}else 
{this.clientMethodUndefined("RTSL_ObjectUnavailable");}}
function SL_NR(A){}
function SL_IC(){}
function SL_NF(A){if(typeof RTSL_ObjectReadDenied=="function"){RTSL_ObjectReadDenied(A);}else 
{this.clientMethodUndefined("RTSL_ObjectReadDenied");}}
function SL_IB(A){if(typeof RTSL_ObjectWriteDenied=="function"){RTSL_ObjectWriteDenied(A);}else 
{this.clientMethodUndefined("RTSL_ObjectWriteDenied");}}
function SL_IO(B,A){SL4B_Logger.alert(B+" is undefined."+((typeof A!="undefined") ? " "+A : ""));}
...
function RTSL_ContribObject(C,D,A,B){var l_oContributionFieldData=SL_PJ.getContributionFieldData(C);
l_oContributionFieldData.addField(D,A);if(B){var l_oContribSubscriber=new SL_IS(C,l_oContributionFieldData);
SL_PJ.removeObjectFromQueue(C);return l_oContribSubscriber.m_nIdentifier;
}return null;
}
...
SL_PJ = new function(){this.m_pContributionDataQueue=new Object();this.getContributionFieldData = function(A){if(typeof this.m_pContributionDataQueue[A]=="undefined"){this.m_pContributionDataQueue[A]=new SL4B_ContributionFieldData();}return this.m_pContributionDataQueue[A];
};
...
SL_HP = new function(){this.m_nCurrentUniqueIdentifier=1;this.getUniqueIdentifier = function(){return this.m_nCurrentUniqueIdentifier++;
};
};
function SL_IS(B,A){this.m_nIdentifier=SL_HP.getUniqueIdentifier();this.m_sObjectName=B;this.m_oContributionFieldData=A;this.initialise();}
SL_IS.prototype = new SL4B_AbstractSubscriber;SL_IS.prototype.ready = SL_RB;SL_IS.prototype.getIdentifier = function(){return this.m_nIdentifier;
};
SL_IS.prototype.contribOk = SL_BW;SL_IS.prototype.contribFailed = SL_MB;function SL_RB(){SL4B_Accessor.getRttpProvider().contribObject(this,this.m_sObjectName,this.m_oContributionFieldData);}
function SL_BW(A){if(typeof RTSL_ContribOk!="undefined"){RTSL_ContribOk(this.m_nIdentifier,A);}else 
...
function SL_MB(A,B){if(typeof RTSL_ContribFailed!="undefined"){RTSL_ContribFailed(this.m_nIdentifier,A,B);}else 
...
SL_IS.prototype.getContributionIdentifier = function(){return this.m_nIdentifier;
};
...
function SL_RS(B,A){var l_oRttpProvider=SL4B_Accessor.getRttpProvider();
...
function RTSL_AddFlashType(G,E,J,R,K,A,O,D,H,B,L,P,Q,I,N,C,F,M){g_oUpdateFlash.m_sFlashTypes[G]=new SL_OF(E,J,R,K,A,O,D,H,B,L,P,Q,I,N,C,F,M);}
...
function SL_OF(E,I,Q,J,A,N,D,G,B,K,O,P,H,M,C,F,L){this.m_fEvalUpdate=E;this.m_oUp=new SL_BX(I,N,K,M);this.m_oDown=new SL_BX(Q,D,O,C);this.m_oEqual=new SL_BX(J,G,P,F);this.m_oFresh=new SL_BX(A,B,H,L);}
function SL_BX(D,B,A,C){this.m_sFlashClr=RTSL_DefaultValue(D,'');this.m_sFlashText=RTSL_DefaultValue(B,'white');this.m_sFinalText=RTSL_DefaultValue(A,this.m_sFlashClr);this.m_sFinalClr=RTSL_DefaultValue(C,'');this.m_sDefaultFlashClr=this.m_sFlashClr;this.m_sDefaultFlashText=this.m_sFlashText;this.m_sDefaultFinalText=this.m_sFinalText;this.m_sDefaultFinalClr=this.m_sFinalClr;}
SL_BX.prototype.m_RestoreDefaults = SL_PW;SL_BX.prototype.m_SetColours = SL_MA;function SL_PW(){this.m_sFlashClr=this.m_sDefaultFlashClr;this.m_sFlashText=this.m_sDefaultFlashText;this.m_sFinalText=this.m_sDefaultFinalText;this.m_sFinalClr=this.m_sDefaultFinalClr;}
function SL_MA(A){this.m_sFlashClr=A.m_sFlashClr;this.m_sFlashText=A.m_sFlashText;this.m_sFinalText=A.m_sFinalText;this.m_sFinalClr=A.m_sFinalClr;}
function SL_NT(A,C,B,D){var l_oUpdateFlash=g_oUpdateFlash.m_sFlashTypes[A].m_oFresh;
...
g_pPreviousValue[D]=A;l_nOld=GF_ConvertToDecimal(l_sPrevVal);l_nNew=GF_ConvertToDecimal(A);return SL_NT(B,l_nOld,l_nNew,l_sPrevVal);
}
...
var l_oFinalUpdate=SL_NT(C,0,GF_ConvertToDecimal(D),'0.00');
...
if(l_oElement.type=='button'){l_oElement.value=(A.m_bDisplayCommas) ? (SL_QG(A.m_sValue)) : (A.m_sValue);}else 
{switch(A.m_sDivElement){
case 'innerHTML':l_oElement.innerHTML=(A.m_bDisplayCommas) ? (SL_QG(A.m_sValue)) : (A.m_sValue);break;
case 'nodeValue':case 'innerText':l_oElement.childNodes[0].nodeValue=(A.m_bDisplayCommas) ? (SL_QG(A.m_sValue)) : (A.m_sValue);break;
default :l_oElement.innerText=(A.m_bDisplayCommas) ? (SL_QG(A.m_sValue)) : (A.m_sValue);break;
...
SL_CH.prototype.m_Add = SL_DL;SL_CH.prototype.m_Remove = SL_NP;SL_CH.timeoutID=null;function SL_DL(A,C,B){if(typeof (this.m_pQueue[B])=='undefined'){this.m_pQueue[B]=new Array();}var l_nIndex=this.m_pQueue[B].length;
if(l_nIndex==0&&SL_CH.timeoutID==null){SL_CH.timeoutID=setTimeout('C_ClearUpdateFlashes('+B+')',B);}else 
{this.m_Remove(A);}this.m_pQueue[B][l_nIndex]=new SL_AJ(A,C,B);this.m_pLookup[A]=new SL_EJ(B,l_nIndex);}
function SL_NP(A){if(typeof (this.m_pLookup[A])!='undefined'){var l_nFlashTime=this.m_pLookup[A].m_nFlashTime;
...
MOD_CreateImages.prototype.addImage = SL_JJ;function SL_JJ(A,B){this.m_pImageMappings[A]=new Image();this.m_pImageMappings[A].src=B;}
function SL_QG(A){if(isNaN(A)){return A;
}var l_sAbsVal=new String(Math.abs(A));
l_sAbsVal=l_sAbsVal.split('.');var l_sComma='';
var l_nPosition;
...
SL4B_UpdateBatcher = new function(){this.initializeMemberVariables = function(){this.m_nQuantisation=100;this.m_nMaxProcessorUtilisation=0.5;this.m_mBinIdToUpdateBinMap=SL_BE.createMap();this.m_mClearedElementIdToBinIdMap=SL_BE.createMap();this.m_bStarted=false;this.m_bIsRunning=false;};
...
if(l_nBinId){this.m_mClearedElementIdToBinIdMap=SL_BE.removeItem(this.m_mClearedElementIdToBinIdMap,A);for(var x=0,l=l_nBinId.length;x<l;x++){if(l_nBinId[x]!==undefined&&this.m_mBinIdToUpdateBinMap[l_nBinId[x]]!==undefined){delete this.m_mBinIdToUpdateBinMap[l_nBinId[x]][A];}else 
...
this.deleteUpdateBin = function(A){this.m_mBinIdToUpdateBinMap=SL_BE.removeItem(this.m_mBinIdToUpdateBinMap,A);};
...
var SL4B_ScriptLoader = new function(){this.const_STANDARD_CREDENTIALS_PROVIDER="standard";this.const_KEYMASTER_CREDENTIALS_PROVIDER="keymaster";this.m_oIndexScript=null;this.m_sRelativeUrlPrefix="";this.m_bLogFileNames=false;this.findIndexScript=SL_MK;this.getRelativeUrlPrefix=SL_OP;this.getRootUrl = function(){return this.m_sRelativeUrlPrefix;
};
this.loadRelativeScript=SL_HW;this.loadScript=SL_BL;this.loadRequiredScripts=SL_HY;this.loadConfiguredScripts=SL_HO;this.setCommonDomain=SL_SF;this.createRttpProvider=SL_AI;this.createSnapshotProvider=SL_QK;this.getIndexScript = function(){return this.m_oIndexScript;
};
...
SL4B_ScriptLoader.const_OBJECT_RTTP_PROVIDER="object";SL4B_ScriptLoader.const_APPLET_RTTP_PROVIDER="applet";SL4B_ScriptLoader.const_JAVASCRIPT_RTTP_PROVIDER="javascript";SL4B_ScriptLoader.const_TEST_RTTP_PROVIDER="test";function SL_MK(){var l_oIndexScript=null;
...
function SL_OP(){var l_sRelativeUrlPrefix="";
...
function SL_HW(A){this.loadScript(this.m_sRelativeUrlPrefix+A);}
function SL_BL(A,B){if(this.m_bLogFileNames){SL4B_Logger.log(SL4B_DebugLevel.const_DEBUG_INT,"ScriptLoader.loadScript: Loading {0}",A);}var tag=document.createElement('script');
...
}SL4B_Accessor.getLogger().openDebugConsoleOnStartUp(true);C_CallbackQueue.start();this.m_bLogFileNames=true;if(this._mustLoadCredentialsScripts(A)){if(A.getCredentialsProvider()==this.const_STANDARD_CREDENTIALS_PROVIDER){SL_RV();}else 
if(A.getCredentialsProvider()==this.const_KEYMASTER_CREDENTIALS_PROVIDER){SL_CX();}else 
{this.loadScript(A.getCredentialsProvider());}}if(this._mustLoadSlaveFrameProvider(A)){}if(this._mustLoadSlaveFrameWrapperProvider(A)){}if(false){}if(A.getService()!=null){SL_JV();}if(A.getRttpProvider()==SL4B_ScriptLoader.const_APPLET_RTTP_PROVIDER){}else 
...
{this.loadScript(A.getRttpProvider());}SL_FG();if(A.includeRtml()){}if(A.includeRtsl()||A.includeFlash()){}if(A.includeRtsl()){SL_IX();}if(A.includeFlash()){}}
function SL_SF(A){if(window.G_UNIT_TESTING){return true;
...
{l_oProvider=SL4B_Accessor.getRttpProviderFactory().createRttpProvider(new SL4B_SimpleLiberatorConfiguration());}if(l_oProvider!=null){SL4B_Accessor.setRttpProvider(l_oProvider);if(l_bSetUnderlyingRttpProvider){SL4B_Accessor.setUnderlyingRttpProvider(l_oProvider);}l_oProvider.addConnectionListener(new SL4B_LogConnectionListener(SL4B_Logger));SL_PP(l_oProvider);}}
function SL_QK(){var l_oSnapshotProvider=new SL4B_SnapshotProvider();
SL4B_Accessor.setSnapshotProvider(l_oSnapshotProvider);}
function SL_PP(B){B.addConnectionListener(SL4B_ConnectionEventRecorder);var fAddConnectionListener=B.addConnectionListener;
...
SL4B_Throwable.prototype.initialise = SL_KY;SL4B_Throwable.prototype.getClass = function(){return this.m_sClassName;
};
...
function SL_KY(A,B){this.m_sClassName=A;this.m_sMessage=B;var l_fCaller=SL_KY.caller;
...
}SL4B_Version = new function(){this.m_sVersionNumber="@version@";this.m_sBuildNumber="@build.number@";this.m_sBuildDate="@build.date@";this.m_sCopyright="@copyright@";this.getVersion = function(){return this.m_sVersionNumber;
};
